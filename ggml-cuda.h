...
GGML_API void   ggml_stream_synchronize(const int stream);
GGML_API void   ggml_cuda_cpy_ptr_1d(struct ggml_tensor *dst, const struct ggml_tensor *src, int host_i, int device_i);
...
GGML_API void ggml_compute_forward_sync_point(
    const struct ggml_compute_params * params,
    const struct ggml_tensor * src0,
    struct ggml_tensor * dst
);

extern void * gpu_start;
extern void * gpu_stop;
#ifdef  __cplusplus
}
#endif
