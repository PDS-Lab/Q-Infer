...
static __global__ void dequantize_mul_mat_axpy_sparse_batch(const void * __restrict__ vx, const dfloat * __restrict__ y, float * __restrict__ dst, const int ncols, const int nrows, int src1_ne0, int src1_ncols, int *lst, float *idx) {
    // qk = quantized weights per x block
    // qr = number of quantized weights per data value in x block
    // vx gpu子矩阵 weight[n_embed, n_ffn_gpu_sliced]
    // y 输入 input[n_ffn, batch_size]
    // dst 结果
    // printf("%d %d %d %d\n", ncols, nrows, src1_ne0, src1_ncols);
    // weight[n_embed, n_ffn_gpu_sliced], input[n_ffn, batch_size]
    // eg. [8192, 512] [32768, 64]
    const int gpu_row = blockIdx.y*blockDim.y + threadIdx.y; // gpu子矩阵第几行
...
template <int qk, int qr, dequantize_kernel_t dequantize_kernel>
static __global__ void dequantize_mul_mat_axpy_sparse_batch_optimize(const void * __restrict__ vx, const dfloat * __restrict__ y, float * __restrict__ dst, const int ncols, const int nrows, int src1_ne0, int src1_ncols, int *lst, float *idx) {
    const int gpu_row = blockIdx.y*blockDim.y + threadIdx.y; // gpu子矩阵第几行
    const int col_id = blockIdx.z;
    ...
    float * loop_idx = idx + col_id * src1_ne0;
    dfloat * loop_y = (dfloat *)y + col_id * src1_ne0;
    float * loop_dst = dst + col_id * ncols;
...
// partial sum for each thread
    ...
    if (loop_idx[row] < dev_sparse_threshold && threadIdx.y != 0) {
        return;
    }
    __syncthreads();
    for (int i = 0; i < ncols; i += iter_stride) {
...
//     float * loop_dst = dst;
//     // printf("%d %d %d %d\n", ncols, nrows, src1_ne0, src1_ncols);
//     // weight[n_embed, n_ffn_gpu], input[n_ffn, batch_size]
//     // eg. [8192, 512] [32768, 64]
//     // const int local_size = ncols / GGML_CUDA_DMMV_X;
//     float partial_sum[256];
...
//         {
//             // i=0
//             // tid=0  col=0  ib=0 iqs=0  iybs=0  0  16
//             // tid=1  col=2  ib=0 iqs=1  iybs=0  1  17
//             // tid=2  col=4  ib=0 iqs=2  iybs=0  2  18
//             // ...
//             // tid=15 col=30 ib=0 iqs=15 iybs=0  15 31
//             // tid=16 col=32 ib=1 iqs=0  iybs=32 32 48
//             // ...
//             // tid=31 col=62 ib=1 iqs=15 iybs=32 47 63
//             const int col = i + vals_per_iter * tid;
//             const int ib = (gpu_row * ncols + col) / qk; // x block index
//             const int iqs = (col % qk) / qr;         // x quant index
//             const int iybs = col - col % qk;         // y block start index
...
//                 partial_sum[i / iter_stride * 2] = v.x * loop_y[row];
//                 partial_sum[i / iter_stride * 2 + 1] = v.y * loop_y[row];
//                 // atomicAdd(&loop_dst[iybs + iqs + j / qr + 0], v.x * loop_y[row]);
//                 // atomicAdd(&loop_dst[iybs + iqs + j / qr + y_offset], v.y * loop_y[row]);
//             }
//         }
//         /* __syncthreads(); */
//         // __syncthreads();
...
//             const int iqs = (col % qk) / qr;         // x quant index
//             const int iybs = col - col % qk;         // y block start index
//             atomicAdd(&loop_dst[iybs + iqs + 0], partial_sum[i / iter_stride * 2]);
//             atomicAdd(&loop_dst[iybs + iqs + y_offset], partial_sum[i / iter_stride * 2 + 1]);
//             partial_sum[i / iter_stride * 2] = 0;
//             partial_sum[i / iter_stride * 2 + 1] = 0;
//         }
//         loop_dst += ncols;
//         loop_idx += src1_ne0;
//         loop_y += src1_ne0;
...
static __global__ void soft_max_max_f32(const float * x, float * dst, const int ncols) {
    const int row = blockDim.x*blockIdx.x + threadIdx.x;
    const int block_size = blockDim.y;
    const int tid = threadIdx.y;
...
static __global__ void soft_max_sum_f32(const float * x, float * dst, const int ncols) {
    const int row = blockDim.x*blockIdx.x + threadIdx.x;
    const int block_size = blockDim.y;
    const int tid = threadIdx.y;
...
static __global__ void soft_max_exp_f32(const float * x, const float * y, float * dst, const int ncols) {
    const int row = blockDim.x*blockIdx.x + threadIdx.x;
    const int block_size = blockDim.y;
    const int tid = threadIdx.y;
    ...
    for (int col = tid; col < ncols; col += block_size) {
        const int i = row*ncols + col;
        const float val = expf(x[i] - y[row]);
        dst[i] = val;
    }
}

static __global__ void soft_max_merge_f32(const float * kqv0, const float * kqv1,
                                          const float * max0, const float * max1,
                                          const float * sum0, const float * sum1,
    ...
    float mx0 = max0[row];
    float mx1 = max1[row];
    float mx = max(mx0, mx1);

    float exp_mxa = expf(mx0 - mx);
    float exp_mxb = expf(mx1 - mx);
    ...
    for (int col = tid; col < ncols; col += block_size) {
        const int i = row*ncols + col;
        float tmp = (kqv0[i] * exp_mxa) + (kqv1[i] * exp_mxb);
        tmp /= (l0 * exp_mxa + l1 * exp_mxb);
        dst[i] = tmp;
    }
}
...
static void dequantize_axpy_sparse_batch_q4_0_cuda(const void * vx, const dfloat * y, float * dst, const int ncols, const int nrows, int src1_rows, int src1_ncols, cudaStream_t stream, int *lst, float *idx) {
    ...
    // batch维度并行 && 线程块内神经元32线程先合并结果
    // const int Y_SIZE = 32;
    // const int block_num_y = (nrows + Y_SIZE - 1) / Y_SIZE;
    // // const int block_num_y = (nrows + GGML_CUDA_MMV_Y - 1) / GGML_CUDA_MMV_Y;
    // const dim3 block_nums(1, block_num_y, src1_ncols);
    // const dim3 block_dims(WARP_SIZE, Y_SIZE, 1);
    // // const dim3 block_dims(WARP_SIZE, GGML_CUDA_MMV_Y, 1);
    // dequantize_mul_mat_axpy_sparse_batch_optimize<QK4_0, QR4_0, dequantize_q4_0>
    //     <<<block_nums, block_dims, ncols*sizeof(float), stream>>>(vx, y, dst, ncols, nrows, src1_rows, src1_ncols, lst, idx);
}
// static void dequantize_axpy_sparse_batch_q4_0_cuda(const void * vx, const dfloat * y, float * dst, const int ncols, const int nrows, int src1_rows, int src1_ncols, cudaStream_t stream, int *lst, float *idx) {
//     GGML_ASSERT(ncols % GGML_CUDA_DMMV_X == 0);
//     const int block_num_x = ncols / GGML_CUDA_DMMV_X / 2;
//     // const int block_num_y = (nrows + GGML_CUDA_MMV_Y - 1) / GGML_CUDA_MMV_Y;
...
static void soft_max_max_f32_cuda(const float * x, float * dst, const int ncols_x, const int nrows_x, cudaStream_t stream) {
    const dim3 block_dims(1, WARP_SIZE, 1);
    const dim3 block_nums(nrows_x, 1, 1);
    soft_max_max_f32<<<block_nums, block_dims, 0, stream>>>(x, dst, ncols_x);
}

static void soft_max_sum_f32_cuda(const float * x, float * dst, const int ncols_x, const int nrows_x, cudaStream_t stream) {
    const dim3 block_dims(1, WARP_SIZE, 1);
    const dim3 block_nums(nrows_x, 1, 1);
    soft_max_sum_f32<<<block_nums, block_dims, 0, stream>>>(x, dst, ncols_x);
}

static void soft_max_exp_f32_cuda(const float * x, const float * y, float * dst, const int ncols_x, const int nrows_x, cudaStream_t stream) {
    const dim3 block_dims(1, WARP_SIZE, 1);
    const dim3 block_nums(nrows_x, 1, 1);
    soft_max_exp_f32<<<block_nums, block_dims, 0, stream>>>(x, y, dst, ncols_x);
}

static void soft_max_merge_f32_cuda(const float * kqv0, const float * kqv1,  
                                    const float * max0, const float * max1,
                                    const float * sum0, const float * sum1,
                                    float * dst, const int ncols_x, const int nrows_x, cudaStream_t stream) {
    const dim3 block_dims(1, WARP_SIZE, 1);
    const dim3 block_nums(nrows_x, 1, 1);
    soft_max_merge_f32<<<block_nums, block_dims, 0, stream>>>(kqv0, kqv1, max0, max1, sum0, sum1, dst, ncols_x);
}
...
void ggml_init_cublas() {
    ...
    if (!initialized) {
        ...
        g_device_count = min(1, g_device_count);
...
void ggml_stream_synchronize(const int stream) {
    CUDA_CHECK(cudaStreamSynchronize(g_cudaStreams[g_main_device][stream]));
}

void ggml_cuda_cpy_ptr_1d(struct ggml_tensor *dst, const struct ggml_tensor *src, int host_i, int device_i) {
    // ggml_cuda_set_device(g_main_device);
    const cudaStream_t load_stream = g_cudaStreams[g_main_device][1];
    ...
    // cudaError_t ret = cudaMemcpy(dst_ptr, src_ptr, ne0 * ggml_type_size(type)/blck, cudaMemcpyHostToDevice);
    cudaError_t ret =
        cudaMemcpyAsync(dst_ptr, src_ptr, ne0 * ggml_type_size(type) / blck, cudaMemcpyHostToDevice, load_stream);
    CUDA_CHECK(ret);
}
...
inline void ggml_cuda_op_mul_mat_batch_sparse(
    ...
    // printf("dst_tensor: %s, sparse_tensor: %s, backend: %d, sparse_idx: %p, sparse_tensor.data: %p, "
    //        "sparse_tensor.extra: %p\n",
    //        dst->name, dst->src[2]->name, dst->src[2]->backend, sparse_idx, dst->src[2]->data, dst->src[2]->extra);
    // dst_tensor: ffn_up_sparse_gpu-0, sparse_tensor: mlp_pre_out-0, backend: 0(CPU), 
    // sparse_idx: 0x7f9a50020d60, sparse_tensor.data: 0x7f9a50020d60, sparse_tensor.extra: (nil)

    // ggml_tensor *sparse_tensor = dst->src[2];
    // size_t sparse_size;
    // if (sparse_tensor->backend == GGML_BACKEND_CPU) {
    //     float *sparse_dd = (float *)ggml_cuda_pool_malloc(ggml_nbytes(sparse_tensor), &sparse_size);
    //     sparse_idx = sparse_dd;
    //     cudaMemcpyAsync(sparse_idx, sparse_tensor->data, sparse_size, cudaMemcpyHostToDevice, stream);
    // }
    ...
    // if (sparse_tensor->backend == GGML_BACKEND_CPU) {
    //     ggml_cuda_pool_free(sparse_idx, sparse_size);
    // }
...
inline void ggml_cuda_op_soft_max_max(
    const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst,
    const float * src0_dd, const float * src1_dd, float * dst_dd, const cudaStream_t & main_stream) {
    ...
    soft_max_max_f32_cuda(src0_dd, dst_dd, ne00, nrows, main_stream);
...
inline void ggml_cuda_op_soft_max_sum(
    const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst,
    const float * src0_dd, const float * src1_dd, float * dst_dd, const cudaStream_t & main_stream) {
    ...
    soft_max_sum_f32_cuda(src0_dd, dst_dd, ne00, nrows, main_stream);
...
inline void ggml_cuda_op_soft_max_exp(
    const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst,
    const float * src0_dd, const float * src1_dd, float * dst_dd, const cudaStream_t & main_stream) {
    ...
    soft_max_exp_f32_cuda(src0_dd, src1_dd, dst_dd, ne00, nrows, main_stream);
...
static void ggml_cuda_op_flatten(const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst, const ggml_cuda_op_flatten_t op) {
    ...
    if (strncmp(dst->name, "ffn", 3) == 0) {
        // CUDA_CHECK(cudaStreamSynchronize(g_cudaStreams[g_main_device][0]));
        print_time = false;
    }
    int64_t time_load_input = 0;
    int64_t time_compute = 0;
    int64_t time_unload_result = 0;
    ...
    if (print_time) {
        ...
        time_load_input += now - start_time;
        start_time = now;
    }
...
static void ggml_cuda_soft_max_merge(const ggml_tensor *src0, const ggml_tensor *src1, ggml_tensor * dst) {
    const struct ggml_tensor *kqv0 = dst->src[0];
    const struct ggml_tensor *kqv1 = dst->src[3];
    GGML_ASSERT(ggml_is_contiguous(kqv0));
    GGML_ASSERT(ggml_is_contiguous(kqv1));
    GGML_ASSERT(ggml_are_same_shape(kqv0, kqv1));
    GGML_ASSERT(ggml_are_same_shape(kqv0, dst));

    const struct ggml_tensor *max0 = dst->src[1];
    const struct ggml_tensor *max1 = dst->src[4];
    GGML_ASSERT(ggml_is_contiguous(max0));
    GGML_ASSERT(ggml_is_contiguous(max1));
    GGML_ASSERT(ggml_are_same_shape(max0, max1));

    const struct ggml_tensor *sum0 = dst->src[2];
    const struct ggml_tensor *sum1 = dst->src[5];
    GGML_ASSERT(ggml_is_contiguous(sum0));
    GGML_ASSERT(ggml_is_contiguous(sum1));
    GGML_ASSERT(ggml_are_same_shape(sum0, sum1));
    GGML_ASSERT(ggml_are_same_shape(max0, sum1));

    GGML_ASSERT(kqv0->type == GGML_TYPE_F32);
    GGML_ASSERT(kqv1->type == GGML_TYPE_F32);
    GGML_ASSERT(max0->type == GGML_TYPE_F32);
    GGML_ASSERT(max1->type == GGML_TYPE_F32);
    GGML_ASSERT(sum0->type == GGML_TYPE_F32);
    GGML_ASSERT(sum1->type == GGML_TYPE_F32);
    GGML_ASSERT(dst->type == GGML_TYPE_F32);

    const int64_t nrows0 = ggml_nrows(kqv0);
    const int64_t nrows1 = ggml_nrows(max0);
    const int64_t nrows2 = ggml_nrows(sum0);

    ggml_tensor_extra_gpu * kqv0_extra =            (ggml_tensor_extra_gpu *) kqv0->extra;
    ggml_tensor_extra_gpu * kqv1_extra =            (ggml_tensor_extra_gpu *) kqv1->extra;

    ggml_tensor_extra_gpu * max0_extra =            (ggml_tensor_extra_gpu *) max0->extra;
    ggml_tensor_extra_gpu * max1_extra =            (ggml_tensor_extra_gpu *) max1->extra;

    ggml_tensor_extra_gpu * sum0_extra =            (ggml_tensor_extra_gpu *) sum0->extra;
    ggml_tensor_extra_gpu * sum1_extra =            (ggml_tensor_extra_gpu *) sum1->extra;

    ggml_tensor_extra_gpu * dst_extra  =            (ggml_tensor_extra_gpu *)  dst->extra;

    const bool kqv0_on_device = kqv0->backend == GGML_BACKEND_GPU || kqv0->backend == GGML_BACKEND_GPU_SPLIT;
    const bool kqv1_on_device = kqv1->backend == GGML_BACKEND_GPU || kqv1->backend == GGML_BACKEND_GPU_SPLIT;
    const bool max0_on_device = max0->backend == GGML_BACKEND_GPU || max0->backend == GGML_BACKEND_GPU_SPLIT;
    const bool max1_on_device = max1->backend == GGML_BACKEND_GPU || max1->backend == GGML_BACKEND_GPU_SPLIT;
    const bool sum0_on_device = sum0->backend == GGML_BACKEND_GPU || sum0->backend == GGML_BACKEND_GPU_SPLIT;
    const bool sum1_on_device = sum1->backend == GGML_BACKEND_GPU || sum1->backend == GGML_BACKEND_GPU_SPLIT;
    const bool  dst_on_device = dst->backend == GGML_BACKEND_GPU;

    // dd = data device
    float * kqv0_ddf = nullptr;
    float * kqv1_ddf = nullptr;
    float * max0_ddf = nullptr;
    float * max1_ddf = nullptr;
    float * sum0_ddf = nullptr;
    float * sum1_ddf = nullptr;
    float *  dst_ddf = nullptr;
    ...
    // move data
    if (kqv0_on_device) {
        kqv0_ddf = (float *) kqv0_extra->data_device[g_main_device];
    } else {
        kqv0_ddf = (float *) ggml_cuda_pool_malloc(ggml_nbytes(kqv0), &kqv0_asf);
        CUDA_CHECK(ggml_cuda_cpy_tensor_2d(kqv0_ddf, kqv0, 0, 0, 0, nrows0, main_stream));
    }

    if (kqv1_on_device) {
        kqv1_ddf = (float *) kqv1_extra->data_device[g_main_device];
    } else {
        kqv1_ddf = (float *) ggml_cuda_pool_malloc(ggml_nbytes(kqv1), &kqv1_asf);
        CUDA_CHECK(ggml_cuda_cpy_tensor_2d(kqv1_ddf, kqv1, 0, 0, 0, nrows0, main_stream));
    }

    if (max0_on_device) {
        max0_ddf = (float *) max0_extra->data_device[g_main_device];
    } else {
        max0_ddf = (float *) ggml_cuda_pool_malloc(ggml_nbytes(max0), &max0_asf);
        CUDA_CHECK(ggml_cuda_cpy_tensor_2d(max0_ddf, max0, 0, 0, 0, nrows1, main_stream));
    }

    if (max1_on_device) {
        max1_ddf = (float *) max1_extra->data_device[g_main_device];
    } else {
        max1_ddf = (float *) ggml_cuda_pool_malloc(ggml_nbytes(max1), &max1_asf);
        CUDA_CHECK(ggml_cuda_cpy_tensor_2d(max1_ddf, max1, 0, 0, 0, nrows1, main_stream));
    }

    if (sum0_on_device) {
        sum0_ddf = (float *) sum0_extra->data_device[g_main_device];
    } else {
        sum0_ddf = (float *) ggml_cuda_pool_malloc(ggml_nbytes(sum0), &sum0_asf);
        CUDA_CHECK(ggml_cuda_cpy_tensor_2d(sum0_ddf, sum0, 0, 0, 0, nrows2, main_stream));
    }

    if (sum1_on_device) {
        sum1_ddf = (float *) sum1_extra->data_device[g_main_device];
    } else {
        sum1_ddf = (float *) ggml_cuda_pool_malloc(ggml_nbytes(sum1), &sum1_asf);
        CUDA_CHECK(ggml_cuda_cpy_tensor_2d(sum1_ddf, sum1, 0, 0, 0, nrows2, main_stream));
    }
    ...
    // do the computation
    soft_max_merge_f32_cuda(
        kqv0_ddf, kqv1_ddf, 
        max0_ddf, max1_ddf, 
        sum0_ddf, sum1_ddf, 
        dst_ddf, dst->ne[0], nrows0, main_stream
    );
#ifdef TRUE_RUNTIME
    ...
    if (kqv0_asf > 0) {
        ggml_cuda_pool_free(kqv0_ddf, kqv0_asf);
    }
    if (kqv1_asf > 0) {
        ggml_cuda_pool_free(kqv1_ddf, kqv1_asf);
    }
    if (max0_asf > 0) {
        ggml_cuda_pool_free(max0_ddf, max0_asf);
    }
    if (max1_asf > 0) {
        ggml_cuda_pool_free(max1_ddf, max1_asf);
    }
    if (sum0_asf > 0) {
        ggml_cuda_pool_free(sum0_ddf, sum0_asf);
    }
    if (sum1_asf > 0) {
        ggml_cuda_pool_free(sum1_ddf, sum1_asf);
    }
    if (dst_asf > 0) {
        ggml_cuda_pool_free(dst_ddf, dst_asf);
    }
...
static void ggml_cuda_cpy_cont(const ggml_tensor * src0, const ggml_tensor * src1) {
    const int64_t ne00 = src0->ne[0];
    const int64_t ne01 = src0->ne[1];
    const int64_t ne02 = src0->ne[2];
    const int64_t ne03 = src0->ne[3];
    ...
    for (int64_t i0 = 0; i0 < ne13*ne12; ++i0) {
        ...
        if (src1->backend == GGML_BACKEND_GPU) {
            ...
            dst = (char *)src1_extra->data_device[g_main_device];
        } else {
            dst = (char *)src1->data;
        }

        char  *  dst_i = dst + (i0) * ne01*ne00*src0_ts/src0_bs;

        CUDA_CHECK(ggml_cuda_cpy_tensor_2d(dst_i, src0, i03, i02, 0, ne01, stream));
    }
}
...
static void ggml_cuda_op_mul_mat(
    ...
    for (int64_t id = 0; id < g_device_count; ++id) {
        ...
        if (src0_on_device && src0_is_contiguous) {
            ...
            size_t sts = ggml_time_us();
            const size_t size_src0_ddq = split ? (row_high[id]-row_low[id])*ne00 * src0_ts/src0_bs : ggml_nbytes(src0);
            src0_dd[id] = (char *) ggml_cuda_pool_malloc(ggml_nbytes(src0), &src0_as[id]);
            size_t ets = ggml_time_us();
            stat_record(global_stat.mem_stat, "malloc_src0", ets-sts, dst->backend, GGML_BACKEND_GPU);
        }
        ...
        if (src1_on_device && src1_is_contiguous) {
            ...
            size_t ets = ggml_time_us();
            stat_record(global_stat.mem_stat, "malloc_src1", ets-sts, dst->backend, GGML_BACKEND_GPU);
        }
        ...
        if (dst_on_device) {
            ...
            size_t ets = ggml_time_us();
            stat_record(global_stat.mem_stat, "malloc_dst", ets-sts, dst->backend, GGML_BACKEND_GPU);
        }
    }
        ...
        for (int64_t id = 0; id < g_device_count; ++id) {
            ...
            for (int64_t i0 = 0; i0 < ne13*ne12; ++i0) {
                ...
                if (src1->backend == GGML_BACKEND_GPU && src1_is_contiguous) {
                    ...
                    size_t ets = ggml_time_us();
                    stat_record(global_stat.mem_stat, "cpy_src1", ets-sts, dst->backend, GGML_BACKEND_GPU);
                ...
                if (src1_col_0 == 0 && (!src0_on_device || !src0_is_contiguous) && i02 % i02_divisor == 0) {
                    ...
                    size_t ets = ggml_time_us();
                    stat_record(global_stat.mem_stat, "cpy_src0", ets-sts, dst->backend, GGML_BACKEND_GPU);
                }
                if (print_time) {
                    CUDA_CHECK(cudaStreamSynchronize(g_cudaStreams[g_main_device][0]));
                    int64_t now = ggml_time_us();
                    ...
                    // [8192, n_ffn_gpu], [up(8192)/down(32768), batch_size]
                }
...
#endif
                size_t ets = ggml_time_us();
                stat_record(global_stat.op_stat_gpu, "GPU_COMPUTATION", ets-sts, dst->backend, GGML_BACKEND_GPU);
                ...
                if (!dst_on_device) {
                    ...
                    if (split) {
                        ...
                        size_t ets = ggml_time_us();
                        stat_record(global_stat.mem_stat, "cpy_dst", ets-sts, dst->backend, GGML_BACKEND_GPU);
                    }
                }
                ...
                if (print_time) {
                    CUDA_CHECK(cudaStreamSynchronize(g_cudaStreams[g_main_device][0]));
                    int64_t now = ggml_time_us();
                    time_unload_result += now - start_time;
                    start_time = now;
                }
    ...
    if (dst->backend == GGML_BACKEND_CPU) {
        // printf("synchronize op_mul_mat %s\n", dst->name);
        CUDA_CHECK(ggml_cuda_set_device(g_main_device));
        // CUDA_CHECK(cudaDeviceSynchronize());
        CUDA_CHECK(cudaStreamSynchronize(g_cudaStreams[g_main_device][0]));
    }
    ...
    if (print_time) {
        printf("%s, time_load_input: %ld, time_compute: %ld, time_unload_result: %ld\n", dst->name, time_load_input, time_compute, time_unload_result);
    }
    if (strncmp(dst->name, "benchmark_result", 16) == 0) {
        // printf("%s[%ld,%ld,%ld,%ld], %s[%ld,%ld,%ld,%ld], %s[%ld,%ld,%ld,%ld]\n",
...
static void ggml_cuda_mul_mat(const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    // if(strncmp(ggml_get_name(dst), "mlp_pre_out", 11) == 0){
        // printf("dst backend: %d\n", dst->backend);
    // }
...
struct neuron_pair {
    int no;
    double score;
    bool operator<(const neuron_pair& b) const {
...
#include <queue>
static void neuron_cache_simu(const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    int layer = dst->layer;
    int neuron_num = dst->src[2]->ne[0];    // 总共多少个神经元
    int batch = src1->ne[1];                // batch维度
    int nrow = dst->src[3]->ne[0];          // GPU上有多少个神经元
    float *sparse_pred = static_cast<float *>(ggml_cuda_get_tensor_data(dst->src[2]));      // CPU上
    if (layer == 0) g_cycle++;
    // update freq
    bool sparse_act_map[STAT_MAX_FFN_NUERON_NUM] = {};
    auto &neuron_freq = g_neuron_cache.freq[layer];
    for (int j = 0; j < batch; j++) {
        for (int i = 0; i < neuron_num; i++) {
            if (sparse_pred[i + j * neuron_num] > 0) {
                neuron_freq.history_freq[i]++;
                neuron_freq.last_freq[i]++;
                sparse_act_map[i] = true;
            }
            if (g_cycle > 10) {
                neuron_freq.last_freq[i] -= neuron_freq.last_window[i][neuron_freq.pos];
            }
            neuron_freq.last_window[i][neuron_freq.pos] = sparse_pred[i] > 0 ? 1 : 0;
        }
    }
    neuron_freq.pos++;
    neuron_freq.pos %= 10;
    // update cache
    auto & neuron_map = g_neuron_cache.meta[layer].neuron_map;
    if (src1->ne[1] > g_batch_size || src1->ne[1] < g_batch_size / 2) {
        return;   
    }

    // count stat
    {
        // recall, gpu上缓存的部分召回了多少激活神经元
        float offload_ratio = nrow * 1.0 / neuron_num;
        int recall_cnt = 0;
        int activate_cnt = 0;
        for (int i = 0; i < neuron_num; i++) {
            for (int j = 0; j < batch; j++) {
                if (sparse_pred[j * neuron_num + i] >= dev_sparse_threshold) {
                    if (neuron_map[i]) {
                        recall_cnt++;
                    }
                    activate_cnt++;
        ...
        float ratio = 1.0 * recall_cnt / activate_cnt;
        g_sparsity.recall.ratio[layer] += ratio;

        int hist_bucket = ratio * 100 / 5;
        g_sparsity.recall.hist[layer][hist_bucket]++;

        // gain
        float gain = ratio / offload_ratio;
        g_sparsity.recall_gain.ratio[layer] += gain;
    }

    if (layer > 0) {
        // victim
        int K = g_neuron_cache.layer_k[layer];
        std::priority_queue<neuron_pair> act_topk;
        std::priority_queue<neuron_pair> nact_topk;
        for (int i = 0; i < neuron_num; i++) {
            if (neuron_map[i]) {
                neuron_pair np = {i, freq_score(neuron_freq.history_freq[i], neuron_freq.last_freq[i])};
                if (sparse_act_map[i] > 0) {
                    if (act_topk.size() < K) {
                        act_topk.push(np);
                    } else if (act_topk.top() > np) {
                        act_topk.pop();
                        act_topk.push(np);
                    }
                } else {
        ...
        std::vector<neuron_pair> act_s;
        std::vector<neuron_pair> nact_s;

        while(!act_topk.empty()) {
            act_s.push_back(act_topk.top());
            act_topk.pop();
        }

        while(!nact_topk.empty()) {
            nact_s.push_back(nact_topk.top());
            nact_topk.pop();
        }

        int cnt = 0;
        for (int i = 0; i < nact_s.size() && cnt < K; i++) {
            neuron_map[nact_s[i].no] = false;
            cnt++; 
        }
        for (int i = 0; i < act_s.size() && cnt < K; i++) {
            neuron_map[act_s[i].no] = false;
            cnt++;
        }
    }

    // prefetch
    if (layer < g_neuron_cache.layer - 1) {
        int K = g_neuron_cache.layer_k[layer + 1];
        auto &next_neuron_map = g_neuron_cache.meta[layer + 1].neuron_map;
        auto &next_neuron_freq = g_neuron_cache.freq[layer + 1];
        std::priority_queue<neuron_pair, std::vector<neuron_pair>, std::greater<neuron_pair>> prefetch_topk;
        for (int i = 0; i < neuron_num; i++) {
            if (sparse_act_map[i] > 0 && !next_neuron_map[i]) {
                neuron_pair np = {i, freq_score(next_neuron_freq.history_freq[i], next_neuron_freq.last_freq[i])};
                if (prefetch_topk.size() < K) {
                    prefetch_topk.push(np);
                } else if (prefetch_topk.top() < np) {
                    prefetch_topk.pop();
                    prefetch_topk.push(np);
                }
            }
        ...
        while(!prefetch_topk.empty()) {
            auto np = prefetch_topk.top();
            next_neuron_map[np.no] = true;
            prefetch_topk.pop();
        }
    }
}

static void static_sparsity_tally(const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    if (src1->ne[1] > g_batch_size || src1->ne[1] < g_batch_size / 2) {
        return;
    }
    int layer = dst->layer;
    ...
    int32_t *row_lookup = static_cast<int32_t *>(ggml_cuda_host_malloc(ggml_nbytes(dst->src[3])));  // GPU上
    CUDA_CHECK(cudaMemcpy(row_lookup, ggml_cuda_get_tensor_data(dst->src[3]), ggml_nbytes(dst->src[3]), cudaMemcpyDeviceToHost));

    int32_t *gpu_indx = static_cast<int32_t *>(ggml_cuda_get_tensor_data(g_gpu_indx[layer]));   // CPU上
    ...
    float offload_ratio = nrow * 1.0 / neuron_num;
    {
        // active ratio, gpu上缓存的神经元被用到的比例
        int active_cnt = 0;
        for (int i = 0; i < nrow; i++) {
            for (int bs = 0; bs < batch; bs++) {
                if (sparse_pred[bs * neuron_num + row_lookup[i]] >= dev_sparse_threshold) {
                    active_cnt++;
                    break;
                }
            }
        }
        float ratio = 1.0 * active_cnt / nrow;
        g_sparsity.active.ratio[layer] += ratio;

        int hist_bucket = ratio * 100 / 5;
        g_sparsity.active.hist[layer][hist_bucket]++;
    }
    {
        // precision, gpu上缓存的神经元真正卸载了多少计算量
        int prec_cnt = 0;           // GPU承担了多少的神经元激活
        int computation_cnt = 0;    // 整个计算中有多少次神经元激活
        for (int j = 0; j < batch; j++) {
            for (int i = 0; i < neuron_num; i++) {
                if (sparse_pred[j * neuron_num + i] >= dev_sparse_threshold) {
                    if (gpu_indx[i] == 1) {
        ...
        float ratio = 1.0 * prec_cnt / computation_cnt;
        g_sparsity.precision.ratio[layer] += ratio;

        int hist_bucket = ratio * 100 / 5;
        g_sparsity.precision.hist[layer][hist_bucket]++;

        // gain
        float gain = ratio / offload_ratio;
        g_sparsity.offload_gain.ratio[layer] += gain;
    }
    {
        // recall, gpu上缓存的部分召回了多少激活神经元
        int recall_cnt = 0;
        ...
        // 联合上下文稀疏性
        float sparsity = 1.0 - 1.0 * activate_cnt / neuron_num;
        g_sparsity.sparsity.ratio[layer] += sparsity;
        hist_bucket = sparsity * 100 / 5;
        g_sparsity.sparsity.hist[layer][hist_bucket]++;
    }
    ggml_cuda_host_free(row_lookup);
}
...
static void ggml_cuda_mul_mat_sparse(const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    // TODO: 统计稀疏激活情况
#ifdef SPARSE_TEST
    static_sparsity_tally(src0, src1, dst);
#elif defined (NEURON_CACHE_SIM)
    if (std::strncmp(dst->name, "ffn_up_sparse_gpu", std::strlen("ffn_up_sparse_gpu")) == 0) {
        neuron_cache_simu(src0, src1, dst);
    }
#endif
...
#endif
        return ;
    } else if (src0->backend == GGML_BACKEND_CPU && src1->backend == GGML_BACKEND_GPU) {
        ggml_cuda_cpy_cont(src0, src1);
        cudaStreamSynchronize(main_stream);
        return ;
    } else if (src0->backend == GGML_BACKEND_CPU && src1->backend == GGML_BACKEND_CPU){
        GGML_ASSERT(-1);
    }
...
static void ggml_cuda_soft_max_max(const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ggml_cuda_op_flatten(src0, src1, dst, ggml_cuda_op_soft_max_max);
}

static void ggml_cuda_soft_max_exp(const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ggml_cuda_op_flatten(src0, src1, dst, ggml_cuda_op_soft_max_exp);
}

static void ggml_cuda_soft_max_sum(const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ggml_cuda_op_flatten(src0, src1, dst, ggml_cuda_op_soft_max_sum);
}
...
static void ggml_cuda_timer_start(const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    (void) src0;
    (void) src1;
    (void) dst;
...
static void ggml_cuda_timer_stop(const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    (void) src0;
    (void) src1;
    (void) dst;
    ...
    CUDA_CHECK(ggml_cuda_set_device(g_main_device));
    const cudaStream_t main_stream = g_cudaStreams[g_main_device][0];
    CUDA_CHECK(cudaEventRecord((cudaEvent_t)gpu_stop, main_stream));
}

void ggml_compute_forward_sync_point(
    ...
    (void) src0;
    (void) dst;
    CUDA_CHECK(cudaEventSynchronize((cudaEvent_t)gpu_stop));
    float elapsedTime;
    CUDA_CHECK(cudaEventElapsedTime(&elapsedTime, (cudaEvent_t)gpu_start, (cudaEvent_t)gpu_stop));

    // 清理事件
    CUDA_CHECK(cudaEventDestroy((cudaEvent_t)gpu_start));
    CUDA_CHECK(cudaEventDestroy((cudaEvent_t)gpu_stop));

    size_t time_us = elapsedTime * 1000;
    stat_record(global_stat.layer_stat[dst->layer], "GPU_time_use", time_us, GGML_BACKEND_GPU, GGML_BACKEND_CPU);

    g_kvc_split.ratio[dst->layer] = update_ratio(g_kvc_split.ratio[dst->layer], cpu_stop - cpu_start, time_us);
}
...
bool ggml_cuda_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    if ((!src0_on_device && (tensor->op == GGML_OP_MUL_MAT_SPARSE || tensor->op == GGML_OP_AXPY))
        || (!any_on_device && tensor->op != GGML_OP_MUL_MAT && tensor->exec_place != GGML_BACKEND_GPU)) {
        return false;
    }
    ...
    // if(strncmp(tensor->name, "l_out-0", 7) == 0 && params->ith == 0){
    //     CUDA_CHECK(cudaDeviceSynchronize());

    //     struct ggml_tensor* down_result = tensor->src[0];

    //     GGML_ASSERT(down_result->backend == GGML_BACKEND_CPU);
    //     int64_t n_token = down_result->ne[1];
    //     int64_t n_embd = down_result->ne[0];

    //     printf("\ntensor %s, backend: %d, type: %d, shape: [%ld, %ld, %ld, %ld] data: %p, extra: %p\n", down_result->name, down_result->backend, down_result->type, down_result->ne[0], down_result->ne[1], down_result->ne[2], down_result->ne[3], down_result->data, down_result->extra);
    //     for(int i = 0; i < n_token; i++){
    //         for(int j = 0; j < 3; j++){
    //             printf("%f ", ((float *)down_result->data)[i*n_embd + j]);
    //         }
    //         printf(" ... ");
    //         for(int j = n_embd-3; j < n_embd; j++){
    //             printf("%f ", ((float *)down_result->data)[i*n_embd + j]);
    //         }
    //         printf("\n");
    //     }

    //     down_result = down_result->src[0];

    //     GGML_ASSERT(down_result->backend == GGML_BACKEND_CPU);
    //     n_token = down_result->ne[1];
    //     n_embd = down_result->ne[0];
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_SOFT_MAX:
            ...
            break;
        case GGML_OP_SOFT_MAX_MAX:
            func = ggml_cuda_soft_max_max;
            break;
        case GGML_OP_START:
            func = ggml_cuda_timer_start;
            break;
        case GGML_OP_STOP:
            func = ggml_cuda_timer_stop;
            break;
        case GGML_OP_SOFT_MAX_EXP:
            func = ggml_cuda_soft_max_exp;
            break;
        case GGML_OP_SOFT_MAX_SUM:
            func = ggml_cuda_soft_max_sum;
            break;
        case GGML_OP_SOFT_MAX_MERGE:
            func = ggml_cuda_soft_max_merge;
            break;
        case GGML_OP_ROPE:
            func = ggml_cuda_rope;
            break;
    ...
    tensor->exec_place = GGML_BACKEND_GPU;
...
static void ggml_backend_cuda_synchronize(ggml_backend_t backend) {
    // printf("synchronize in ggml_backend_cuda_synchronize\n");
    CUDA_CHECK(cudaStreamSynchronize(g_cudaStreams[g_main_device][0]));
