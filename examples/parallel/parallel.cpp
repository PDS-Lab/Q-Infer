...
int main(int argc, char ** argv) {
...

    {
        ...
        int start_pos = 0;
        while (start_pos < n_tokens_system) {
            llama_batch_clear(batch);
            for (int32_t i = 0; i < n_tokens_system && i < params.n_batch; ++i, ++start_pos) {
                llama_batch_add(batch, tokens_system[start_pos], start_pos, { 0 }, false);
        }
        ...
        for (int32_t i = 0; i < (int32_t) batch.n_tokens; i += n_batch) {
            ...
            for (auto & client : clients) {
                ...
                if (client.n_decoded > 2 &&
                    ...
                    LOG_TEE("\033[31mClient %3d, seq %3d/%3d, prompt %4d t, response %4d t, total time %5.2f s, gen time %5.2f, speed %5.2f t/s, cache miss %d \033[0m \nInput:    %s\n\033[35mResponse: %s\033[0m\n\n",
                            client.id, client.seq_id, n_seq, client.n_prompt, client.n_decoded,
                            (t_main_end - client.t_start_prompt) / 1e6,
                            (t_main_end - client.t_start_gen) / 1e6,
