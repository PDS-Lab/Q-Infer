...
def load_activation_weights(models_base: Path):
    ...
    *_, files = next(os.walk(models_base))
    ...
    sorted_filenames = sorted(activation_files, key=lambda x: int(re.search(r'_(\d+)\.pt$', x).group(1)))
    return [torch.load(models_base / f) for f in sorted_filenames]
...
def export_split(activations_path: str, output_path: str, solved_list, vram_capacity: int):
