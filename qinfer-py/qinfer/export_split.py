...
def append_gpu_idx(gguf: GGUFWriter, i_layer: int, activation, select_count, suffix) -> None:
    ...
    key = f"blk.{i_layer}.gpu_idx" + suffix
    print(
    ...
    key = f"blk.{i_layer}.gpu_bucket" + suffix
    print(
...
def export_split(activations_path: str, output_path: str, solved_list: List[List[int]], vram_capacity: int):
    ...
    for i, (activation, selected_count) in enumerate(zip(predictors, solved_list)):
        append_gpu_idx(gguf_out, i, activation, selected_count[0], '')
        append_gpu_idx(gguf_out, i, activation, selected_count[1], '_down')
