...
if __name__ == "__main__":
    ...
    parser.add_argument('--computility-ratio-up', type=float, required=True, help='Computility ratio between GPU and CPU in up-projection')
    parser.add_argument('--computility-ratio-down', type=float, required=True, help='Computility ratio between GPU and CPU in down-projection')
    parser.add_argument('--has-gate', type=int, required=True, help='Whether the model has a gate layer.')
    ...
    solved = solve_gpu_split(
        ...
        computility_ratio_up=args.computility_ratio_up,
        computility_ratio_down=args.computility_ratio_down,
        has_gate=args.has_gate
    )

    print(f"solved: {solved}, total neurons: {sum(sum(x) for x in solved)}")
