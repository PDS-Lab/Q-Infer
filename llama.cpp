...
static size_t llama_set_vram_budget(double budget_gb, int gpu_device) {
    ...
    if (!ggml_cann_loaded()) {
        throw std::runtime_error("cann is not loaded");
    }

    if (budget_gb < 0) {
        // if the user didn't specify a budget, use all available memory
        // and leave 256 MB as a safety margin
        vram_budget_bytes = ggml_cann_get_free_memory(gpu_device) - 256 * 1024 * 1024;
    } else {
        // otherwise, use the specified budget
        vram_budget_bytes = (size_t) (budget_gb * 1024 * 1024 * 1024);
    }
...
#if not defined(GGML_USE_CUBLAS) and not defined(GGML_USE_CANN)
...
#ifdef LOG_NODE_TIME
    FILE *log_out = fopen("qinfer.log", "a");
#endif

    for (int i = 0; i < graph->n_nodes; i++) {
#ifdef LOG_NODE_TIME
        if (strncmp(graph->nodes[i]->name, "ffn", 3) == 0) {
            fprintf(log_out, "node %d: %s, backend: %s, time used: %dus\n", i, graph->nodes[i]->name,
                    graph->nodes[i]->backend == GGML_BACKEND_CPU ? "CPU" : "GPU", graph->nodes[i]->perf_time_us);
        }
#endif
    }
...
#elif GGML_USE_CANN
    if (ggml_cann_loaded()) {
        return ggml_cann_host_malloc(n);
    } else {
        return malloc(n);
    }
#else
...
#elif GGML_USE_CANN
    if (ggml_cann_loaded()) {
        return ggml_cann_host_free(ptr);
    } else {
        return free(ptr);
    }
#else
...
struct llama_hparams {
    ...
    uint32_t n_ff;
    int      kvcache_mode;
    float    kvcache_ratio;     // parallel-attention模式下的CPU计算的kv cache比例
...
struct llama_layer {
    ...
    struct ggml_tensor * ffn_down_t_b;
    ...
    struct ggml_tensor * gpu_idx_down; // index of ffn down neurons on GPU
    ...
    struct ggml_tensor * gpu_bucket_down; // double index from GPU split neuron to original neuron
    // pre_gate
    struct ggml_tensor * pred_idx;
    // sparse idx predicted in the previous layer
    struct ggml_tensor * pre_pred_idx;
};
...
struct llama_model {
    ...
    ~llama_model() {
        ...
        if (ggml_cann_loaded()) {
            for (size_t i = 0; i < tensors_by_name.size(); ++i) {
                ggml_cann_free_data(tensors_by_name[i].second);
            }
...
int store_in_gpu_step;
int store_in_cpu_step;
    ...
    void load_all_data(struct ggml_context * ctx, llama_progress_callback progress_callback, void * progress_callback_user_data, llama_mlock * lmlock) {
        ...
        for (int i = 0; i < gguf_get_n_tensors(ctx_gguf); i++) {
            ...
            switch (cur->backend) {
                ...
                case GGML_BACKEND_GPU:
                case GGML_BACKEND_GPU_SPLIT:
                    ggml_cann_transform_tensor(cur->data, cur);
                    if (!use_mmap) {
                        free(cur->data);
                    }
                    break;
...
struct llama_gpu_split_loader {
    ...
    llama_gpu_split_loader(const std::string & fname, bool use_mmap) : fname(fname), use_mmap(use_mmap) {
        ...
        // tensor_meta_size *= 2; // for gpu_idx_down and gpu_bucket_down
        struct ggml_init_params params = {
    ...
    int load_gpu_idx_for_model(llama_model * model) {
        ...
        for (int il = 0; il < n_layers; il++) {
            ...
            ggml_tensor * gpu_idx = idx_loader->get_tensor_meta(il*4);
            ggml_tensor * gpu_bucket = idx_loader->get_tensor_meta(il*4+1);
            ggml_tensor * gpu_idx_down = idx_loader->get_tensor_meta(il*4+2);
            ggml_tensor * gpu_bucket_down = idx_loader->get_tensor_meta(il*4+3);
            ...
            model_layer.gpu_idx_down = idx_loader->create_tensor_for(ctx_meta, gpu_idx_down, GGML_BACKEND_CPU);
            model_layer.gpu_bucket_down = idx_loader->create_tensor_for(ctx_meta, gpu_bucket_down, GGML_BACKEND_CPU);

            // model_layer.gpu_idx_down = ggml_dup_tensor(ctx_meta, model_layer.gpu_idx);
            // model_layer.gpu_idx_down->data = (uint8_t*)malloc(ggml_nbytes(model_layer.gpu_idx_down));
            // model_layer.gpu_bucket_down = ggml_dup_tensor(ctx_meta, model_layer.gpu_bucket);
            // model_layer.gpu_bucket_down->data = (uint8_t*)malloc(ggml_nbytes(model_layer.gpu_bucket_down));
        }
        ...
        for (int il = 0; il < n_layers; il++) {
            ...
            g_gpu_indx[il] = gpu_idx;
            ggml_tensor * gpu_idx_down = model_layer.gpu_idx_down;
            // memcpy(gpu_idx_down->data, gpu_idx->data, ggml_nbytes(gpu_idx));
            ggml_tensor * gpu_bucket = model_layer.gpu_bucket;
            int64_t gpu_neurons = sum_gpu_index(gpu_idx);
            int32_t *gpu_indx = static_cast<int32_t *>(g_gpu_indx[il]->data);
            auto & neuron_map = g_neuron_cache.meta[il].neuron_map;
            for (int i = 0; i < g_ffn_neuron_num; i++) {
                if (gpu_indx[i] == 1) {
                    neuron_map[i] = true;
                }
            }
            g_neuron_cache.layer_k[il] = gpu_neurons / 2;
            g_neuron_cache.meta[il].cache_size = gpu_neurons;
            model_layer.gpu_offload_ratio = (double)gpu_neurons / gpu_idx->ne[0];
            ggml_tensor * gpu_bucket_down = model_layer.gpu_bucket_down;
            // memcpy(gpu_bucket_down->data, gpu_bucket->data, ggml_nbytes(gpu_bucket));
            gpu_neurons = sum_gpu_index(gpu_idx) + sum_gpu_index(gpu_idx_down);
            model_layer.gpu_offload_ratio = (double)gpu_neurons / (2 * gpu_idx->ne[0]);
            if (gpu_neurons == 0 || gpu_neurons == gpu_idx->ne[0] * 2) {
                // no hybrid inference for this layer, unset gpu_bucket
                model_layer.gpu_bucket = NULL;
                model_layer.gpu_bucket_down = NULL;
                ...
                ggml_set_backend(gpu_bucket_down, GGML_BACKEND_GPU);
                ggml_cuda_transform_tensor(gpu_bucket_down->data, gpu_bucket_down);
#elif defined(GGML_USE_CANN)
                ggml_set_backend(gpu_bucket, GGML_BACKEND_GPU);
                ggml_cann_transform_tensor(gpu_bucket->data, gpu_bucket);

                ggml_set_backend(gpu_bucket_down, GGML_BACKEND_GPU);
                ggml_cann_transform_tensor(gpu_bucket_down->data, gpu_bucket_down);
#else
...
struct llama_augmentation_model_loader {
    ...
    ggml_tensor * create_striped_mat_to_gpu(struct ggml_tensor *src, struct ggml_tensor * gpu_bucket) {
        ...
        *ggml_cuda_get_data_pp(device_mat_row) = *ggml_cuda_get_data_pp(gpu_dst);
        ...
        if (gpu_bucket == NULL) {
            // offload the whole tensor to gpu
            ggml_set_backend(src, GGML_BACKEND_GPU);
            ggml_cann_transform_tensor(src->data, src);
            return src;
        }
        ...
        ggml_tensor * gpu_dst = ggml_new_tensor_2d(aux_ctx, src->type, row_len, gpu_rows);
        ggml_set_backend(gpu_dst, GGML_BACKEND_GPU);
        ggml_cann_alloc_tensor(gpu_dst);

        // init two 1d views on host and device
        ggml_tensor * host_mat_row = ggml_new_tensor_1d(aux_ctx, src->type, row_len);
        static ggml_tensor * device_mat_row = ggml_dup_tensor(aux_ctx, host_mat_row);
        ggml_set_backend(device_mat_row, GGML_BACKEND_GPU);
        ggml_cann_alloc_tensor(device_mat_row);
        *ggml_cann_get_data_pp(device_mat_row) = *ggml_cann_get_data_pp(gpu_dst);
        ...
        for (int i = 0; i < gpu_rows; i++) {
            int32_t host_i = ((int32_t *)gpu_bucket->data)[i];
            host_mat_row -> data = (char *)(src -> data) + host_i * row_data_size;
            char ** gpu_data_pp = reinterpret_cast<char **>(ggml_cann_get_data_pp(device_mat_row));
            // printf("gpu_data_p: %p\n", *gpu_data_pp);
            ggml_cann_cpy_1d(device_mat_row, host_mat_row);
            *gpu_data_pp = *gpu_data_pp + row_data_size;
        }
        ggml_set_no_alloc(aux_ctx, false);
    ...
    size_t slice_ffn_mat_to_gpu(llama_layer & layer) {
        ...
        ggml_tensor * gpu_bucket_down = layer.gpu_bucket_down;
        ...
        layer.ffn_down_gpu = create_striped_mat_to_gpu(layer.ffn_down_t, gpu_bucket_down);
...
#elif defined (GGML_USE_CANN)
        ggml_cann_set_device_constants(model->hparams.sparse_pred_threshold);
#endif
...
#if defined(GGML_USE_CUBLAS) || defined(GGML_USE_CANN)
...
static ggml_cgraph *llm_computility_benchmark_build_graph(int64_t n_embed, int64_t n_ff, int64_t batch_size,
                                                          int64_t n_ff_gpu, ggml_op op, bool use_gpu) {
    ...
    struct ggml_tensor *weight = ggml_new_tensor_2d(ctx0, GGML_TYPE_F16, n_embed, use_gpu ? n_ff_gpu : n_ff);
    struct ggml_tensor *input =
        ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, op == GGML_OP_MUL_MAT_SPARSE ? n_embed : n_ff, batch_size);
    struct ggml_tensor *sparse_idx = ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, n_ff, batch_size);
    struct ggml_tensor *hybrid_idx =
        ggml_new_tensor_1d(ctx0, GGML_TYPE_I32, use_gpu ? n_ff_gpu : n_ff); // gpu_idx or gpu_bucket

    ggml_set_name(weight, "weight");
    ggml_set_name(input, "input");
    ggml_set_name(sparse_idx, "sparse_idx");
    ggml_set_name(hybrid_idx, "hybrid_idx");

    if (op == GGML_OP_MUL_MAT_SPARSE) {
        if (use_gpu) {
            result = ggml_mul_mat_idx_upscale(ctx0, weight, input, sparse_idx, hybrid_idx, n_ff);
            #ifdef GGML_USE_CUBLAS
            ggml_cuda_assign_buffers_no_alloc(result);
            #else
            ggml_cann_assign_buffers_no_alloc(result);
            #endif
            // ggml_set_backend(result, GGML_BACKEND_GPU);

            ggml_set_backend(weight, GGML_BACKEND_GPU);
            ggml_set_backend(sparse_idx, GGML_BACKEND_GPU);
            ggml_set_backend(hybrid_idx, GGML_BACKEND_GPU);
        } else {
            result = ggml_mul_mat_idx(ctx0, weight, input, sparse_idx, hybrid_idx);
        }
    } else if (op == GGML_OP_AXPY) {
        if (use_gpu) {
            result = ggml_axpy(ctx0, weight, input, sparse_idx, hybrid_idx);
            #ifdef GGML_USE_CUBLAS
            ggml_cuda_assign_buffers_no_alloc(result);
            #else
            ggml_cann_assign_buffers_no_alloc(result);
        ...
        GGML_ASSERT(false && "error ggml_op");
    }
    ggml_set_name(result, "benchmark_result");
...
static float llm_computility_benchmark_compute(ggml_context *ctx_data, ggml_allocr *allocr, int n_embd, int n_ff,
                                               int batch_size, int n_ff_gpu, ggml_op op, int need_active_cnt,
                                               bool use_gpu, int n_threads) {
    int active_cnt = 0;
    ggml_allocr_reset(allocr);
    ggml_cgraph *gf = llm_computility_benchmark_build_graph(n_embd, n_ff, batch_size, n_ff_gpu, op, use_gpu);
    ...
    for (int i = 0; i < gf->n_leafs; i++) {
        ...
        if (strcmp(node->name, "weight") == 0) {
            ...
            if(op == GGML_OP_AXPY){
                float *input_data = (float *)node->data;
                for (int bs = 0; bs < batch_size; bs++) {
                    for (int j = 0; j < n_ff / 10; j++) {
                        input_data[bs * n_ff + j] = 1.0;
                    }
                }
            }
        } else if (strcmp(node->name, "sparse_idx") == 0) {
            float *sparse_data = (float *)node->data;
            for (int bs = 0; bs < batch_size; bs++) {
                for (int j = 0; j < (use_gpu ? n_ff_gpu : n_ff); j++) {
                    if (active_cnt < need_active_cnt) { // todo: optimize active distribution
                        sparse_data[bs * n_ff + j] = 1.0;
                        active_cnt++;
                    } else {
                        sparse_data[bs * n_ff + j] = -1.0;
                    }
        ...
        } else if (strcmp(node->name, "hybrid_idx") == 0) {
            if (use_gpu) {
                int32_t *hybrid_data = (int32_t *)node->data;
                for (int j = 0; j < n_ff_gpu; j++) {
                    hybrid_data[j] = j;
                }
            } else {
                int32_t *hybrid_data = (int32_t *)node->data;
        ...
        if (node->backend == GGML_BACKEND_GPU && node->extra == NULL) {
            #ifdef GGML_USE_CUBLAS
            ggml_cuda_assign_scratch_offset(node, (char *)node->data - (char *)ctx_data->mem_buffer);
            ggml_cuda_copy_to_device(node);
            #else
            ggml_cann_assign_scratch_offset(node, (char *)node->data - (char *)ctx_data->mem_buffer);
            ggml_cann_copy_to_device(node);
            #endif
        }
    }
    ...
    int compute_cnt = 3;
    for (int k = 0; k < compute_cnt; k++) {
        ggml_graph_compute(gf, &plan);
    }
    // printf("active cnt: %d\n", active_cnt);
    // printf("compute_time: %ld\n", ggml_time_us() - start_time);

    return 1.0 * active_cnt * compute_cnt / (ggml_time_us() - start_time);
}

#define MAX_LAYER_NUM 80
float computility_up_cpu = 1;
float computility_up_gpu = 8;
float computility_down_cpu = 1;
float computility_down_gpu = 8;
int64_t benchmark_n_batch;

static void llm_computility_benchmark(const struct llama_model *model, float cache_ratio, uint32_t n_thread) {
    if (cache_ratio > 1.0) {
        cache_ratio = 1.0;
    }
    LLAMA_LOG_INFO("llm_computility_benchmark: measuring computility...\n");
    LLAMA_LOG_INFO("cache ratio: %f, cpu thread num: %u, batch_size: %ld\n", cache_ratio, n_thread, benchmark_n_batch);

    int layer_num = model->layers.size();
    int64_t n_embd = model->layers[0].ffn_up->ne[0];
    int64_t n_ff = model->layers[0].ffn_up->ne[1];
    int64_t batch_size = benchmark_n_batch;
    int test_active_cnt = n_ff * batch_size / 10;
    ...
    {
        // worst case graph for memory usage
        ggml_cgraph *gf = llm_computility_benchmark_build_graph(n_embd, n_ff, batch_size, 0, GGML_OP_MUL_MAT_SPARSE, false);
        size_t alloc_size = ggml_allocr_alloc_graph(allocr, gf) + 32;
        allocr = ggml_allocr_new(ctx_data->mem_buffer, alloc_size, 32);
        #ifdef GGML_USE_CUBLAS
        ggml_cuda_set_scratch_size(alloc_size);
        #else
        ggml_cann_set_scratch_size(alloc_size);
        #endif
    }
    ...
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 2; j++) {
            bool use_gpu = i == 1;
            ggml_op op = j == 0 ? GGML_OP_MUL_MAT_SPARSE : GGML_OP_AXPY;
            { // warm up
                int need_active_cnt = n_ff * batch_size;
                llm_computility_benchmark_compute(ctx_data, allocr, n_embd, n_ff, batch_size, n_ff, op, need_active_cnt,
                                                  use_gpu, n_thread);
            }
            float computility_sum = 0;
            for (int layer_i = 5; layer_i < layer_num; layer_i++) {
                int need_active_cnt = test_active_cnt;
                // int64_t n_ff_gpu = model->layers[layer_i].gpu_bucket->ne[0];
                int64_t n_ff_gpu = n_ff * cache_ratio;
                n_ff_gpu = n_ff_gpu / 32 * 32;
                float computility = llm_computility_benchmark_compute(
                    ctx_data, allocr, n_embd, n_ff, batch_size, n_ff_gpu, op, need_active_cnt, use_gpu, n_thread);
                // printf("layer %d(%ld/%ld), computility: %f cnt/us\n", layer_i, n_ff_gpu, n_ff, computility);
                computility_sum += computility;
            }
            if (op == GGML_OP_MUL_MAT_SPARSE) {
                if (use_gpu) {
                    computility_up_gpu = computility_sum / (layer_num - 5);
                } else {
                    computility_up_cpu = computility_sum / (layer_num - 5);
                }
            } else {
                if (use_gpu) {
                    computility_down_gpu = computility_sum / (layer_num - 5);
                } else {
                    computility_down_cpu = computility_sum / (layer_num - 5);
                }
            }
        }
    }
    LLAMA_LOG_INFO("test_active_cnt: %d\n", test_active_cnt);
    LLAMA_LOG_INFO("computility_up_cpu: %f cnt/us\n", computility_up_cpu);
    LLAMA_LOG_INFO("computility_up_gpu: %f cnt/us\n", computility_up_gpu);
    LLAMA_LOG_INFO("computility_down_cpu: %f cnt/us\n", computility_down_cpu);
    LLAMA_LOG_INFO("computility_down_gpu: %f cnt/us\n", computility_down_gpu);
...
static bool load_gpu_split_from_split_file(llama_model & model, std::string split_path, size_t vram_budget) {
    llama_gpu_split_loader loader(split_path, false);
    return loader.check_vram_allocable(vram_budget) 
        && loader.load_gpu_idx_for_model(&model) == 0;
}

static bool llm_load_gpu_split_with_budget(llama_model_loader & ml, llama_model & model, size_t vram_allocatable_bytes, bool no_cache, uint32_t n_thread) {
    ...
    // printf("******: %ld %ld\n", ffn_up->ne[0], ffn_up->ne[1]); [n_embd, n_ffn][8192,32768]
    // why not ffn_up->ne[0]?
    ...
    // int vram_bytes_per_slice = slice_size * (ffn_gate ? 3 : 2); // TODO: why 4.5, not 3?
    int vram_bytes_per_slice = slice_size;
    int neuron_cap = floor((double)vram_allocatable_bytes / vram_bytes_per_slice);
    int total_neuron = ffn_up->ne[1] * model.layers.size() * (ffn_gate ? 3 : 2);

    llm_computility_benchmark(&model, 1.0 * neuron_cap / total_neuron, n_thread);
...
#else
    command_ss << "PYTHONPATH=/home/wq/Q-infer/qinfer-py python3 -m qinfer"
#endif
               ...
               << " --computility-ratio-up " << computility_up_gpu / computility_up_cpu
               << " --computility-ratio-down " << computility_down_gpu / computility_down_cpu
               << " --has-gate " << (ffn_gate ? 1 : 0)
    ...
    if (system(command_ss.str().c_str()) != 0 || access(cached_split_path.c_str(), F_OK) != 0) {
        ...
        GGML_ASSERT(false && "failed to generate gpu split, invoke python module failed");
        return false;
    }
...
static size_t llm_load_gpu_split(llama_model_loader & ml, llama_model & model, bool no_cache, bool no_offload, uint32_t n_thread) {
#if defined (GGML_USE_CUBLAS) || defined (GGML_USE_CANN)
    #if defined(GGML_USE_CUBLAS)
    if (!ggml_cublas_loaded()) {
    ...
    #else
    if (!ggml_cann_loaded()) {
        throw std::runtime_error(format("cannot offload to GPU: " GGML_CANN_NAME " not loaded"));
    }
    #endif
    if (!no_offload && !llm_load_gpu_split_with_budget(ml, model, vram_budget_bytes, no_cache, n_thread)) {
...
static void llm_load_sparse_model_tensors(
...
#elif defined(GGML_USE_CANN)
    if (ggml_cann_loaded()) {
        LLAMA_LOG_INFO("%s: using " GGML_CANN_NAME " for GPU acceleration\n", __func__);
        ggml_cann_set_main_device(main_gpu);
...

    {
        ...
        net_layer_num = n_layer;
        g_neuron_cache.layer = n_layer;
        g_ffn_neuron_num = hparams.n_ff;
        ...
        switch (model.arch) {
            ...
            case LLM_ARCH_FALCON:
             ...
             case LLM_ARCH_OPT:
                {
                    model.tok_embd = create_tensor(tn(LLM_TENSOR_TOKEN_EMBD, "weight"), {n_embd, n_vocab});
                    model.pos_embd = create_tensor(tn(LLM_TENSOR_POS_EMBD,   "weight"), {n_embd, 2050});
                    // output
                    {
                        model.output_norm = create_tensor(tn(LLM_TENSOR_OUTPUT_NORM, "weight"), {n_embd});
                        model.output_norm_b = create_tensor(tn(LLM_TENSOR_OUTPUT_NORM, "bias"),   {n_embd});
                    ...
                    for (uint32_t &i = current_layer; i < n_layer; ++i) {
                       auto & layer = model.layers[i];
                        ...
                        layer.wk = create_tensor(tn(LLM_TENSOR_ATTN_K,   "weight", i), {n_embd, n_embd_gqa});
                        layer.bk = create_tensor(tn(LLM_TENSOR_ATTN_K,   "bias", i), {n_embd});
                        layer.wv = create_tensor(tn(LLM_TENSOR_ATTN_V,   "weight", i), {n_embd, n_embd_gqa});
                        layer.bv = create_tensor(tn(LLM_TENSOR_ATTN_V,   "bias", i), {n_embd});
                        layer.wo = create_tensor(tn(LLM_TENSOR_ATTN_OUT, "weight", i), {n_embd, n_embd});
                        ...
                        layer.ffn_down_t = create_tensor(tn(LLM_TENSOR_FFN_DOWN_T, "weight", i), {n_embd, n_ff});
                        layer.ffn_down_t_b = create_tensor(tn(LLM_TENSOR_FFN_DOWN_T, "bias", i), {n_embd});
                        layer.mlp_pre_w1 = create_tensor(tn(LLM_TENSOR_MLP_PRED_FC1, "weight", i), {n_embd, GGML_NE_WILDCARD});
                        layer.mlp_pre_w2 = create_tensor(tn(LLM_TENSOR_MLP_PRED_FC2, "weight", i), {GGML_NE_WILDCARD, n_ff});
                        layer.ffn_up   = create_tensor(tn(LLM_TENSOR_FFN_UP,   "weight", i), {n_embd,   n_ff});
...
#if defined(GGML_USE_CUBLAS) || defined(GGML_USE_CLBLAST) || defined(GGML_USE_CANN)
    ...
    if (cparams != NULL) {
        ...
        // LLAMA_LOG_INFO("%s: offloaded layers %d/%d\n", __func__, model.n_gpu_layers, hparams.n_layer);
    }
    // Offload FFN segments to GPU if possible
    model.ffn_offloaded_bytes = llm_load_gpu_split(ml, model, reset_gpu_index, disable_ffn_split || !alloc.tensor_offload_complete, cparams->n_threads);
...
#elif defined(GGML_USE_CANN)
    {
        ggml_cann_set_tensor_split(tensor_split);
    }
#endif
...
static bool llama_model_load(const std::string & fname, llama_model & model, const llama_model_params & params, const llama_context_params * cparams) {
    try {
        ...
        model.hparams.kvcache_mode = params.kvcache_mode;
        model.hparams.kvcache_ratio = params.kvcache_ratio;
    ...
    for (int i = 0; i < model.hparams.n_layer; i++) {
        g_kvc_split.ratio[i] = params.kvcache_ratio;
    }
    g_kvc_split.update_step = params.kvcache_step;
...
static struct ggml_tensor * llm_build_sparse_mul_mat(
   ...
   const llm_build_cb_short & cb,
    ...
    if (full_gpu) {
        GGML_ASSERT(up_gpu && "full_gpu but no up_gpu");
        out = ggml_mul_mat_idx(ctx, up_gpu, inp, idx, NULL);
        ggml_cann_assign_buffers_no_alloc(out);
        cb(out, (full_name).c_str());
        return out;
    }
#endif
...
#if defined(GGML_USE_CUBLAS) || defined(GGML_USE_CANN)
    if (up_gpu) {
        ggml_tensor * out_gpu = ggml_mul_mat_idx_upscale(ctx, up_gpu, inp, idx, gpu_bucket, up->ne[1]);
        #ifdef GGML_USE_CUBLAS
        ggml_cuda_assign_buffers_no_alloc(out_gpu);
        #else
        ggml_cann_assign_buffers_no_alloc(out_gpu);
        #endif
...
static struct ggml_tensor * llm_build_ffn_sparse(
        struct ggml_context * ctx,
         ...
         struct ggml_tensor * pre_pred_idx,
         struct ggml_tensor * gpu_index,
         struct ggml_tensor * gpu_index_down,
         struct ggml_tensor * gpu_bucket,
         struct ggml_tensor * gpu_bucket_down,
   ...
   const llm_build_cb_short & cb_outer) {
    ...
    llm_build_cb_short cb = [&cb_outer](struct ggml_tensor * tensor, const char * name) {
        ...
        bool operates_on_gpu = tensor->src[0]->backend == GGML_BACKEND_GPU;
        if (operates_on_gpu) {
            ggml_cann_assign_buffers_no_alloc(tensor);
        }
#endif
    };

    ggml_tensor *idx = NULL;
    if (pre_pred_idx != NULL) {
        idx = pre_pred_idx;
    } else {
    // prepare sparse idx
    ...
    cur = llm_build_sparse_axpy(ctx, down_t, cur, idx, down_gpu, gpu_index_down, gpu_bucket_down, cb_outer, "down", full_gpu);
...
static struct ggml_tensor * llm_build_kqv_parallel(
        struct ggml_context * ctx,
        const llama_hparams & hparams,
       const llama_kv_cache & kv,
         struct ggml_tensor * wo,
    ...
    float ratio = g_kvc_split.ratio[il];
    if (il == 27) {
        add_ratio_curve(ratio);
    }
    int seq_cpu = n_kv * ratio;
    int seq_gpu = n_kv - seq_cpu;

    struct ggml_tensor* kq_mask_gpu = ggml_view_3d(ctx, kq_mask, 
        seq_gpu, n_tokens, 1, ggml_element_size(kq_mask) *n_kv,
                              ggml_element_size(kq_mask) *n_kv * n_tokens, 0);

    struct ggml_tensor* kq_mask_cpu = ggml_view_3d(ctx, kq_mask, 
        seq_cpu, n_tokens, 1, ggml_element_size(kq_mask) *n_kv, 
                              ggml_element_size(kq_mask) *n_kv * n_tokens,
                              ggml_element_size(kq_mask) * seq_gpu);

    struct ggml_tensor * k_gpu_view = ggml_view_3d(ctx, kv.k,
                n_embd_head, seq_gpu, n_head_kv,
                ggml_element_size(kv.k)*n_embd_gqa,
                ggml_element_size(kv.k)*n_embd_head,
                ggml_element_size(kv.k)*n_embd_gqa*n_ctx*il);
    
    struct ggml_tensor *k = ggml_view_3d(ctx, kv.k,
                n_embd_head, seq_cpu, n_head_kv,
                ggml_element_size(kv.k)*n_embd_gqa,
                ggml_element_size(kv.k)*n_embd_head,
                ggml_element_size(kv.k)*n_embd_gqa* (n_ctx*il + seq_gpu));
    cb(k, "k", il);

    // struct ggml_tensor *kcache_gpu = ggml_dup_tensor(ctx, k_gpu_view);
    // kcache_gpu->op = GGML_OP_CPY_TENSOR;
    // ggml_cuda_assign_buffers_no_alloc(kcache_gpu);
    // cb(kcache_gpu, "kcache_gpu", il);

    // kcache_gpu = ggml_cpy(ctx, k_gpu_view, kcache_gpu);
    // cb(kcache_gpu, "kcache_gpu_cpy", il);
    k_gpu_view = ggml_timer_start(ctx, k_gpu_view);
    ggml_cuda_assign_buffers_no_alloc(k_gpu_view);
    k_gpu_view->exec_place = GGML_BACKEND_GPU;
    cb(k_gpu_view, "GPU_time_start", il);

    struct ggml_tensor * kq_gpu = ggml_mul_mat(ctx, k_gpu_view, q);
    ggml_cuda_assign_buffers_no_alloc(kq_gpu);
    cb(kq_gpu, "kq_gpu", il);

    // printf("kq_gpu  K [%ld, %ld, %ld, %ld] * Q [%ld, %ld, %ld, %ld] = [%ld, %ld, %ld, %ld]\n", 
    // kq_gpu->src[0]->ne[0], kq_gpu->src[0]->ne[1], kq_gpu->src[0]->ne[2], kq_gpu->src[0]->ne[3],
    // kq_gpu->src[1]->ne[0], kq_gpu->src[1]->ne[1], kq_gpu->src[1]->ne[2], kq_gpu->src[1]->ne[3],
    // kq_gpu->ne[0], kq_gpu->ne[1], kq_gpu->ne[2], kq_gpu->ne[3]);

    kq_gpu = ggml_scale(ctx, kq_gpu, kq_scale);
    ggml_cuda_assign_buffers_no_alloc(kq_gpu);
    cb(kq_gpu, "kq_scaled_gpu", il);

    kq_gpu = ggml_add(ctx, kq_gpu, kq_mask_gpu);
    ggml_cuda_assign_buffers_no_alloc(kq_gpu);
    cb(kq_gpu, "kq_masked_gpu", il);
    
    struct ggml_tensor *gpu_max = ggml_soft_max_max(ctx, kq_gpu);
    ggml_cuda_assign_buffers_no_alloc(gpu_max);
    cb(gpu_max, "kq_soft_max_max_gpu", il);

    kq_gpu = ggml_soft_max_exp(ctx, kq_gpu, gpu_max);
    ggml_cuda_assign_buffers_no_alloc(kq_gpu);
    cb(kq_gpu, "kq_soft_max_exp_gpu", il);

    struct ggml_tensor *gpu_sum = ggml_soft_max_sum(ctx, kq_gpu);
    ggml_cuda_assign_buffers_no_alloc(gpu_sum);
    cb(gpu_sum, "kq_soft_max_sum_gpu", il);

    gpu_sum = ggml_timer_stop(ctx, gpu_sum);
    ggml_cuda_assign_buffers_no_alloc(gpu_sum);
    gpu_sum->exec_place = GGML_BACKEND_GPU;
    cb(gpu_sum, "GPU_time_stop", il);

    // split cached v into n_head heads
    struct ggml_tensor * v_gpu_view = ggml_view_3d(ctx, kv.v,
                seq_gpu, n_embd_head, n_head_kv,
                ggml_element_size(kv.v)*n_ctx,
                ggml_element_size(kv.v)*n_ctx*n_embd_head,
                ggml_element_size(kv.v)*n_ctx*n_embd_gqa*il);

    struct ggml_tensor * v = ggml_view_3d(ctx, kv.v,
                seq_cpu, n_embd_head, n_head_kv,
                ggml_element_size(kv.v)*n_ctx,
                ggml_element_size(kv.v)*n_ctx*n_embd_head,
                ggml_element_size(kv.v)*(n_ctx*n_embd_gqa*il + seq_gpu));


    // struct ggml_tensor *vcache_gpu = ggml_dup_tensor(ctx, v_gpu_view);
    // vcache_gpu->op = GGML_OP_CPY_TENSOR;
    // ggml_cuda_assign_buffers_no_alloc(vcache_gpu);
    // cb(vcache_gpu, "vcache_gpu", il);

    // vcache_gpu = ggml_cpy(ctx, v_gpu_view, vcache_gpu);
    // ggml_cuda_assign_buffers_no_alloc(vcache_gpu);
    // cb(vcache_gpu, "vcache_gpu_cpy", il);

    struct ggml_tensor * kqv_gpu = ggml_mul_mat(ctx, v_gpu_view, kq_gpu);
    ggml_cuda_assign_buffers_no_alloc(kqv_gpu);
    cb(kqv_gpu, "kqv_gpu", il);
    // printf("kqv_gpu  V [%ld, %ld, %ld, %ld] *  KQ [%ld, %ld, %ld, %ld] = [%ld, %ld, %ld, %ld]\n", 
    // kqv_gpu->src[0]->ne[0], kqv_gpu->src[0]->ne[1], kqv_gpu->src[0]->ne[2], kqv_gpu->src[0]->ne[3],
    // kqv_gpu->src[1]->ne[0], kqv_gpu->src[1]->ne[1], kqv_gpu->src[1]->ne[2], kqv_gpu->src[1]->ne[3],
    // kqv_gpu->ne[0], kqv_gpu->ne[1], kqv_gpu->ne[2], kqv_gpu->ne[3]);

    // struct ggml_tensor * kqv_merged_gpu = ggml_cont_3d(ctx, ggml_permute(ctx, kqv_gpu, 2, 0, 1, 3), n_tokens, n_head, n_embd_head);
    // ggml_cuda_assign_buffers_no_alloc(kqv_merged_gpu);
    // cb(kqv_merged_gpu, "kqv_merged_gpu", il);

    // CPU ATTN
    k = ggml_timer_start(ctx, k);
    cb(k, "CPU_time_start", il);
    ...
    kq = ggml_add(ctx, kq, kq_mask_cpu);
    cb(kq, "kq_masked", il);

    struct ggml_tensor * cpu_max =  ggml_soft_max_max(ctx, kq);
    cb(cpu_max, "kq_soft_max_max", il);

    kq = ggml_soft_max_exp(ctx, kq, cpu_max);
    cb(kq, "kq_soft_max_exp", il);

    struct ggml_tensor * cpu_sum =  ggml_soft_max_sum(ctx, kq);
    cb(cpu_sum, "kq_soft_max_sum", il);

    cpu_sum = ggml_timer_stop(ctx, cpu_sum);
    cb(cpu_sum, "CPU_time_stop", il);

    struct ggml_tensor * kqv = ggml_mul_mat(ctx, v, kq);
    cb(kqv, "kqv", il);

    cpu_sum = ggml_sync_point(ctx, cpu_sum);
    cb(cpu_sum, "sync_point", il);

    struct ggml_tensor *total_attn = ggml_soft_max_merge(ctx, kqv_gpu, gpu_max, gpu_sum, kqv, cpu_max, cpu_sum);
    ggml_cuda_assign_buffers_no_alloc(total_attn);
    cb(total_attn, "total_attn", il);

    total_attn = ggml_permute(ctx, total_attn, 0, 2, 1, 3);
    cb(total_attn, "total_attn_merge", il);

    struct ggml_tensor * cur = ggml_cont_2d(ctx, total_attn, n_embd, n_tokens);
    ggml_cuda_assign_buffers_no_alloc(cur);
    cb(cur, "kqv_merged_cont", il);

    // output
    cur = ggml_mul_mat(ctx, wo, cur);
    // printf("kqv_out [%ld, %ld, %ld, %ld] * [%ld, %ld, %ld, %ld] = [%ld, %ld, %ld, %ld]\n", 
    // cur->src[0]->ne[0], cur->src[0]->ne[1], cur->src[0]->ne[2], cur->src[0]->ne[3],
    // cur->src[1]->ne[0], cur->src[1]->ne[1], cur->src[1]->ne[2], cur->src[1]->ne[3],
...
static struct ggml_tensor * llm_build_kqv_gpu(
        struct ggml_context * ctx,
        const llama_hparams & hparams,
       const llama_kv_cache & kv,
         struct ggml_tensor * wo,
    ...
    struct ggml_tensor *kcache_gpu = ggml_new_tensor_3d(ctx, k->type, n_embd_head, n_kv, n_head_kv);
    kcache_gpu->op = GGML_OP_CPY_TENSOR;
    ggml_cuda_assign_buffers_no_alloc(kcache_gpu);
    cb(kcache_gpu, "kcache_gpu", il);

    kcache_gpu = ggml_cpy(ctx, k, kcache_gpu);
    cb(kcache_gpu, "kcache_gpu_cpy", il);

    struct ggml_tensor * kq = ggml_mul_mat(ctx, kcache_gpu, q);
    ggml_cuda_assign_buffers_no_alloc(kq);
    cb(kq, "kq", il);
    // printf("kq [%ld, %ld, %ld, %ld] * [%ld, %ld, %ld, %ld] = [%ld, %ld, %ld, %ld]\n", 
    // kq->src[0]->ne[0], kq->src[0]->ne[1], kq->src[0]->ne[2], kq->src[0]->ne[3],
    // kq->src[1]->ne[0], kq->src[1]->ne[1], kq->src[1]->ne[2], kq->src[1]->ne[3],
    ...
    struct ggml_tensor *vcache_gpu = ggml_new_tensor_3d(ctx, v->type, n_kv, n_embd_head, n_head_kv);
    vcache_gpu->op = GGML_OP_CPY_TENSOR;
    ggml_cuda_assign_buffers_no_alloc(vcache_gpu);
    cb(vcache_gpu, "vcache_gpu", il);

    vcache_gpu = ggml_cpy(ctx, v, vcache_gpu);
    cb(vcache_gpu, "vcache_gpu_cpy", il);

    struct ggml_tensor * kqv = ggml_mul_mat(ctx, vcache_gpu, kq);
    ggml_cuda_assign_buffers_no_alloc(kqv);
    cb(kqv, "kqv", il);
    // printf("kqv [%ld, %ld, %ld, %ld] * [%ld, %ld, %ld, %ld] = [%ld, %ld, %ld, %ld]\n", 
    // kqv->src[0]->ne[0], kqv->src[0]->ne[1], kqv->src[0]->ne[2], kqv->src[0]->ne[3],
    // kqv->src[1]->ne[0], kqv->src[1]->ne[1], kqv->src[1]->ne[2], kqv->src[1]->ne[3],
    // kqv->ne[0], kqv->ne[1], kqv->ne[2], kqv->ne[3]);

    struct ggml_tensor * kqv_merged = ggml_permute(ctx, kqv, 0, 2, 1, 3);
    ggml_cuda_assign_buffers_no_alloc(kqv_merged);
...
static struct ggml_tensor * llm_build_kqv(
    ...
    if (hparams.kvcache_mode == 1) {
        return llm_build_kqv_gpu(
            ctx, hparams, kv, wo, wo_b, q_cur, kq_scale, 
            kq_mask, n_ctx, n_tokens, n_kv, max_alibi_bias, cb, il);
    } 
    
    if (hparams.kvcache_mode == 2 && n_kv > 256 && g_kvc_split.ratio[il] <= 0.75) {
    // if (hparams.kvcache_mode == 2) {
        return llm_build_kqv_parallel(
            ctx, hparams, kv, wo, wo_b, q_cur, kq_scale, 
            kq_mask, n_ctx, n_tokens, n_kv, max_alibi_bias, cb, il);
    }
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            // feed-forward network
            {
                ...
                if (llama_use_sparse_inference(&model)) {
                    ...
                    llm_build_cb_short cb_sparse = [&cbs](struct ggml_tensor *tensor, const char *name) {
                        cbs(tensor, name);
#if defined(GGML_USE_CUBLAS)
                        // Determine offloading based on src[0] (weight for both mul and axpy)
                        bool operates_on_gpu = tensor->src[0]->backend == GGML_BACKEND_GPU;
                    ...
                    if (5 <= il + 1 && il + 1 < n_layer) {
                        // prepare sparse idx
                        struct ggml_tensor *idx = ggml_mul_mat(ctx0, model.layers[il + 1].mlp_pre_w1, ffn_inp);
                        cb_sparse(idx, "mlp_pre_hidden_cross");
                        idx = ggml_relu(ctx0, idx);
                        cb_sparse(idx, "mlp_pre_relu_cross");
                        idx = ggml_mul_mat(ctx0, model.layers[il + 1].mlp_pre_w2, idx);
                        (model.layers[il + 1].gpu_offload_ratio >= 1.0 ? cb_sparse : cbs)(idx, "mlp_pre_out_cross");

                        ggml_build_forward_expand(gf, idx); // comment this line will be faster?

                        model.layers[il + 1].pre_pred_idx = idx;

                        auto build_transform_buffer = [&](struct ggml_context *ctx, struct ggml_tensor *src,
                                                          struct ggml_tensor *dst) {
                            GGML_ASSERT(src != NULL && dst != NULL && "cached weight tensor in gpu is null");
                            dst->src[0] = src;
                        };

                        build_transform_buffer(ctx0, model.layers[il + 1].ffn_gate, model.layers[il + 1].ffn_gate_gpu);
                        build_transform_buffer(ctx0, model.layers[il + 1].ffn_up, model.layers[il + 1].ffn_up_gpu);
                        build_transform_buffer(ctx0, model.layers[il + 1].ffn_down_t,
                                               model.layers[il + 1].ffn_down_gpu);
                        model.layers[il + 1].ffn_down_gpu->src[1] = model.layers[il + 1].gpu_idx_down;
                        model.layers[il + 1].ffn_down_gpu->src[2] = model.layers[il + 1].gpu_bucket_down;

                        ggml_tensor *empty_tensor = ggml_new_tensor_1d(ctx0, GGML_TYPE_I32, 0);
                        empty_tensor->op = GGML_OP_PRELOAD;
                        empty_tensor->src[0] = idx;
                        empty_tensor->src[1] = model.layers[il + 1].gpu_idx;
                        empty_tensor->src[2] = model.layers[il + 1].gpu_bucket;
                        empty_tensor->src[3] = model.layers[il + 1].ffn_gate_gpu;
                        empty_tensor->src[4] = model.layers[il + 1].ffn_up_gpu;
                        empty_tensor->src[5] = model.layers[il + 1].ffn_down_gpu;
                        cbs(empty_tensor, "neuron_replace");
                        ggml_build_forward_expand(gf, empty_tensor);
                    }
                    if (il == n_layer - 1) { // for synchronize
                    ...
                    cur = llm_build_ffn_sparse(ctx0, cur,
                        ...
                        il >= 5 ? model.layers[il].pre_pred_idx : NULL,
                        model.layers[il].gpu_idx, 
                        model.layers[il].gpu_idx_down,
                        model.layers[il].gpu_bucket, model.layers[il].gpu_bucket_down, model.layers[il].ffn_gate_gpu, model.layers[il].ffn_down_gpu, model.layers[il].ffn_up_gpu,
    ...
    struct ggml_cgraph * build_falcon() {
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            if (llama_use_sparse_inference(&model)) {
                ...
                if (5 <= il + 1 && il + 1 < n_layer && model.layers[il + 1].gpu_offload_ratio < 1.0) {
                    // prepare sparse idx
                    struct ggml_tensor *idx = ggml_mul_mat(ctx0, model.layers[il + 1].mlp_pre_w1, inpL);
                    cb_sparse(idx, "mlp_pre_hidden_cross");
                    idx = ggml_relu(ctx0, idx);
                    cb_sparse(idx, "mlp_pre_relu_cross");
                    idx = ggml_mul_mat(ctx0, model.layers[il + 1].mlp_pre_w2, idx);
                    ...
                    // build_transform_buffer(ctx0, model.layers[il + 1].ffn_gate, model.layers[il + 1].ffn_gate_gpu);
                    build_transform_buffer(ctx0, model.layers[il + 1].ffn_up, model.layers[il + 1].ffn_up_gpu);
                    build_transform_buffer(ctx0, model.layers[il + 1].ffn_down_t, model.layers[il + 1].ffn_down_gpu);
                    model.layers[il + 1].ffn_down_gpu->src[1] = model.layers[il + 1].gpu_idx_down;
                    model.layers[il + 1].ffn_down_gpu->src[2] = model.layers[il + 1].gpu_bucket_down;
                ...
                cur = llm_build_ffn_sparse(ctx0, attn_norm,
                    ...
                    model.layers[il].gpu_bucket_down,
            ...
            struct ggml_tensor * ffn_out = cur;
                ...
                // feed-forward network
                {
                    ...
                    if (llama_use_sparse_inference(&model)) {
                        ...
                        cur = llm_build_ffn_sparse(ctx0, cur,
                            ...
                            NULL, NULL,
                            model.layers[il].ffn_down_t, model.layers[il].ffn_down_t_b,
                            model.layers[il].mlp_pre_w1,
                            model.layers[il].mlp_pre_w2,
                            ffn_inp,
                            il >= 5 ? model.layers[il].pre_pred_idx : NULL,
                            ...
                            model.layers[il].gpu_bucket_down, 
                            NULL,
                            model.layers[il].ffn_down_gpu, 
                            model.layers[il].ffn_up_gpu,
                            LLM_FFN_RELU, LLM_FFN_SEQ, model.layers[il].gpu_offload_ratio, cbs);
                    } else {
                        // GGML_ASSERT(false && "Dense inference is not supported for this model");
                        // fallback to dense
                        cb(cur, "ffn_norm", il);
                        cur = llm_build_ffn(ctx0, cur,
                            model.layers[il].ffn_up,   model.layers[il].ffn_up_b,
                            NULL, NULL,
                            model.layers[il].ffn_down, model.layers[il].ffn_down_t_b,
                            LLM_FFN_RELU, LLM_FFN_SEQ, cb, il);
                    }
                }
...
static const std::unordered_map<const char *, llm_offload_func_e> k_offload_map = {
  ...
  //{ "inp_embd",                   OFFLOAD_FUNC_NR  }, // TODO: missing K-quants get_rows kernel
    ...
    { "inp_pos_opt",                OFFLOAD_FUNC_KQ  }, // this is often used for KQ ops (e.g. rope)
...
static struct ggml_cgraph * llama_build_graph(
    ...
    llm_build_cb cb = [&](struct ggml_tensor * cur, const char * name, int il) {
        ...
        if (!alloc_inp_pos && strcmp(name, "inp_pos_opt") == 0) {
            ggml_allocr_alloc(lctx.alloc, cur);
...
//#define LLAMA_OFFLOAD_DEBUG
...
#elif defined(GGML_USE_CANN)
            { OFFLOAD_FUNC,     "GPU (CANN)" },
            { OFFLOAD_FUNC_KQ,  "GPU (CANN) KQ" },
            { OFFLOAD_FUNC_V,   "GPU (CANN) V" },
            { OFFLOAD_FUNC_NR,  "GPU (CANN) NR" },
            { OFFLOAD_FUNC_EMB, "GPU (CANN) EMB" },
#else
            { OFFLOAD_FUNC,     "CPU" },
            { OFFLOAD_FUNC_KQ,  "CPU" },
            { OFFLOAD_FUNC_V,   "CPU" },
...
#elif defined(GGML_USE_CANNN)
        static offload_func_t ggml_offload_gpu = ggml_cann_assign_buffers_no_alloc;
#else
    ...
    // log graph nodes
    // for(int i=0; i<result->n_nodes; i++) {
    //     LLAMA_LOG_INFO("%s: %s, op: %d\n", __func__, result->nodes[i]->name, result->nodes[i]->op);
    // }
    // exit(0);
...
static int llama_decode_internal(
    ...
    for (int i = 0; i < gf->n_leafs; i++) {
        ggml_tensor * node = gf->leafs[i];
        if (node->backend == GGML_BACKEND_GPU && node->extra == NULL) {
            ggml_cann_assign_scratch_offset(node, (char*)node->data - (char *) lctx.buf_alloc.data);
            ggml_cann_copy_to_device(node);
        }
    }
    ...
    while (true) {
        ...
        if (lora_tensors.find(base_name + ".loraA") != lora_tensors.end() &&
            ...
            if (dest_t->backend == GGML_BACKEND_GPU) {
                if (dest_t->type != GGML_TYPE_F16) {
                    throw std::runtime_error(format(
                        "%s: error: the simultaneous use of LoRAs and NPU acceleration is only supported for f16 models. dest_t->type: %d", __func__, dest_t->type));
                }
                offload_func = ggml_cann_assign_buffers;
                offload_func_force_inplace = ggml_cann_assign_buffers_force_inplace;
            }
#endif // GGML_USE_CUBLAS
...
struct llama_context * llama_new_context_with_model(
    ...
    cparams.batch_size       = params.batch_size;
    ...
    if (!hparams.vocab_only) {
...

        {
            ...
            int n_tokens =  (int)std::max(std::min({cparams.n_ctx, cparams.batch_size, cparams.n_batch}), 128u);
            ...
            LLAMA_LOG_INFO("%s: total VRAM used: %.2f MB (model: %.2f MB, context: %.2f MB, ffn_offload: %.2f)\n", __func__,
                    (total_vram_size + model->ffn_offloaded_bytes) / 1024.0 / 1024.0,
                    model_vram_size / 1024.0 / 1024.0,
                    ctx_vram_size / 1024.0 / 1024.0,
                    model->ffn_offloaded_bytes / 1024.0 / 1024.0);
#endif
        }
...
void llama_statistic(struct ggml_cgraph* cgraph) {
    for (int i = 0; i < cgraph->n_nodes; i++) {
        struct ggml_tensor * node = cgraph->nodes[i];
        if (node->backend != GGML_BACKEND_CPU) {
            stat_record(global_stat.op_stat_gpu, ggml_op_name(node->op), std::max(1l, node->perf_time_us), node->backend, node->exec_place);
        } else {
            stat_record(global_stat.op_stat, ggml_op_name(node->op), std::max(1l, node->perf_time_us), node->backend, node->exec_place);
        }
        int layer = node->layer;
        stat_record(global_stat.layer_stat[layer], ggml_get_name(node), std::max(1l, node->perf_time_us), node->backend, node->exec_place);
    }
}

void llama_show_stat() {
    stat_show(global_stat.op_stat);
    stat_show(global_stat.op_stat_gpu);
    stat_show(global_stat.mem_stat);
    stat_show(global_stat.other_stat);
    for (const auto &layer : global_stat.layer_stat) {
        if (stat_empty(layer)) {
            continue;
        }
        stat_show(layer);
...
void llama_stat_reset() {
    stat_reset(global_stat.op_stat);
    stat_reset(global_stat.op_stat_gpu);
    stat_reset(global_stat.mem_stat);
    stat_reset(global_stat.other_stat);
    for (const auto &layer : global_stat.layer_stat) {
        if (stat_empty(layer)) {
            continue;
        }
