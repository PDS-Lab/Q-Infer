...
struct ggml_tensor * ggml_soft_max_max(
        struct ggml_context * ctx,
        struct ggml_tensor  * a) {
    bool is_node = false;
    ...
    struct ggml_tensor * result = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, a->ne[1], a->ne[2]);

    result->op   = GGML_OP_SOFT_MAX_MAX;
    result->grad = is_node ? ggml_dup_tensor(ctx, result) : NULL;
    result->src[0] = a;
...
struct ggml_tensor * ggml_soft_max_sum(
        struct ggml_context * ctx,
        struct ggml_tensor  * a) {
    bool is_node = false;
    ...
    result->op   = GGML_OP_SOFT_MAX_SUM;
    result->grad = is_node ? ggml_dup_tensor(ctx, result) : NULL;
    result->src[0] = a;
...
struct ggml_tensor * ggml_soft_max_exp(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        struct ggml_tensor  * b) {
    bool is_node = false;
    ...
    result->op   = GGML_OP_SOFT_MAX_EXP;
    result->grad = is_node ? ggml_dup_tensor(ctx, result) : NULL;
    result->src[0] = a;
    result->src[1] = b;
...
struct ggml_tensor * ggml_soft_max_merge(
            struct ggml_context * ctx,
            struct ggml_tensor  * a,
            struct ggml_tensor  * b,
            struct ggml_tensor  * c,
    ...
    GGML_ASSERT(ggml_are_same_shape(b, e));
    GGML_ASSERT(ggml_are_same_shape(c, f));

    struct ggml_tensor * result = ggml_dup_tensor(ctx, a);

    result->op   = GGML_OP_SOFT_MAX_MERGE;
    result->grad = is_node ? ggml_dup_tensor(ctx, result) : NULL;
    result->src[0] = a;
    result->src[1] = b;
    result->src[2] = c;
...
struct ggml_tensor * ggml_timer_start(
            struct ggml_context * ctx,
            struct ggml_tensor  * a) {
...
struct ggml_tensor * ggml_timer_stop(
            struct ggml_context * ctx,
            struct ggml_tensor  * a) {
...
struct ggml_tensor * ggml_sync_point(
            struct ggml_context * ctx,
            struct ggml_tensor  * a) {
...
struct ggml_tensor *preload_src_tensor;
atomic_bool has_new_preload_task = false;
atomic_bool is_over_preload = false;
atomic_bool is_waiting = false;

static void ggml_compute_forward_preload(const struct ggml_compute_params *params, const struct ggml_tensor *src0,
                                         struct ggml_tensor *dst) {
    atomic_store(&is_waiting, true);
    while (true) {
        if (atomic_load(&has_new_preload_task) == false) {
            preload_src_tensor = dst;
            atomic_store(&has_new_preload_task, true);
            // printf("preload add task %s\n", dst->name);
            break;
        }
    }
    atomic_store(&is_waiting, false);
}
...
static void ggml_compute_forward_soft_max_exp_f32(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * src0,
        const struct ggml_tensor * src1,
        struct ggml_tensor * dst) {
    ...
    for (int i1 = ir0; i1 < ir1; i1++) {
        float *sp = (float *)((char *) src0->data + i1*src0->nb[1]);
        float *mx = (float *)((char *) src1->data + i1*src1->nb[0]);
        float *dp = (float *)((char *)  dst->data + i1*dst->nb[1]);
#ifndef NDEBUG
        for (int i = 0; i < nc; ++i) {
            //printf("p[%d] = %f\n", i, p[i]);
        ...
        for (int i = 0; i < nc; i++) {
            if (sp[i] == -INFINITY) {
                ...
                // const float val = (sp[i] == -INFINITY) ? 0.0 : exp(sp[i] - max);
                ggml_fp16_t s = GGML_FP32_TO_FP16(sp[i] - *mx);
                memcpy(&scvt, &s, sizeof(scvt));
                const float val = GGML_FP16_TO_FP32(ggml_table_exp_f16[scvt]);
                dp[i] = val;
            }
...
static void ggml_compute_forward_soft_max_max_f32(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * src0,
        struct ggml_tensor * dst) {
    GGML_ASSERT(ggml_is_contiguous(src0));
    ...
    for (int i1 = ir0; i1 < ir1; i1++) {
        float *sp = (float *)((char *) src0->data + i1*src0->nb[1]);
        float *dp = (float *)((char *)  dst->data + i1*dst->nb[0]);
#ifndef NDEBUG
        for (int i = 0; i < nc; ++i) {
            //printf("p[%d] = %f\n", i, p[i]);
            assert(!isnan(sp[i]));
...
static void ggml_compute_forward_soft_max_sum_f32(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * src0,
        struct ggml_tensor * dst) {
    GGML_ASSERT(ggml_is_contiguous(src0));
...
static void ggml_compute_forward_soft_max_merge_f32(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * src0,
        struct ggml_tensor * dst) {
    GGML_ASSERT(ggml_is_contiguous(src0));
    ...
    for (int i1 = ir0; i1 < ir1; i1++) {
        float *sp0 = (float *)((char *) kqv0->data + i1*kqv0->nb[1]);
        float *sp1 = (float *)((char *) kqv1->data + i1*kqv1->nb[1]);
        float *mx0 = (float *)((char *) max0->data + i1*max0->nb[0]);
        float *mx1 = (float *)((char *) max1->data + i1*max1->nb[0]);
        float *l0 = (float *)((char *) sum0->data + i1*sum0->nb[0]);
        float *l1 = (float *)((char *) sum1->data + i1*sum1->nb[0]);
        ...
        float mxb = *mx1 - mx;
        uint16_t scvt;
        ggml_fp16_t s = GGML_FP32_TO_FP16(mxa);
        memcpy(&scvt, &s, sizeof(scvt));
        const float exp_mxa = GGML_FP16_TO_FP32(ggml_table_exp_f16[scvt]);

        s = GGML_FP32_TO_FP16(mxb);
        memcpy(&scvt, &s, sizeof(scvt));
        const float exp_mxb = GGML_FP16_TO_FP32(ggml_table_exp_f16[scvt]);

        for (int i = 0; i < nc; i++) {
            float tmp = (sp0[i] * exp_mxa + sp1[i] * exp_mxb) / (*l0 * exp_mxa + *l1 * exp_mxb);
            dp[i] = tmp;
#ifndef NDEBUG
            assert(!isnan(dp[i]));
            assert(!isinf(dp[i]));
...
static void ggml_compute_forward_soft_max_exp(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * src0,
        const struct ggml_tensor * src1,
        struct ggml_tensor * dst) {
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_soft_max_exp_f32(params, src0, src1, dst);
            } break;
        default:
            {
                GGML_ASSERT(false);
...
static void ggml_compute_forward_soft_max_max(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * src0,
        struct ggml_tensor * dst) {
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_soft_max_max_f32(params, src0, dst);
            } break;
        default:
            {
                GGML_ASSERT(false);
...
static void ggml_compute_forward_soft_max_sum(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * src0,
        struct ggml_tensor * dst) {
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_soft_max_sum_f32(params, src0, dst);
            } break;
        default:
            {
                GGML_ASSERT(false);
...
static void ggml_compute_forward_soft_max_merge(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * src0,
        struct ggml_tensor * dst) {
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_soft_max_merge_f32(params, src0, dst);
            } break;
        default:
            {
                GGML_ASSERT(false);
...
static void ggml_compute_forward_time_start(
    const struct ggml_compute_params * params,
    const struct ggml_tensor * src0,
    struct ggml_tensor * dst
) {
...
static void ggml_compute_forward_time_stop(
    const struct ggml_compute_params * params,
    const struct ggml_tensor * src0,
    struct ggml_tensor * dst
) {
    cpu_stop = ggml_time_us();
    (void) params;
    (void) src0;
    (void) dst;
    stat_record(global_stat.layer_stat[dst->layer], "CPU_time_use", cpu_stop - cpu_start, GGML_BACKEND_CPU, GGML_BACKEND_CPU);
}
...
atomic_flag g_axpy_q4_lock = ATOMIC_FLAG_INIT;
static void ggml_compute_forward_mul_mat_axpy_q4_0(
    ...
    int64_t t0 = ggml_perf_time_us();
    UNUSED(t0);
    // printf("cpu axpy: src0: %s[%d,%d,%d,%d], src1: %s[%d,%d,%d,%d]\n", src0->name, src0->ne[0], src0->ne[1], src0->ne[2], src0->ne[3], src1->name, src1->ne[0], src1->ne[1], src1->ne[2], src1->ne[3]);
    ...
    const int nr = ggml_nrows(src0);                        // 隐藏层维度

    const int64_t ir10 = dr*ith;                            // 神经元维度开始的位置
    // const int64_t ir11 = MIN(ir10 + dr, src2->ne[0]);

    // src1 rows
    const int64_t nr1 = ne11*ne12*ne13;                     // X总共有多少维度
    float *idx = src2->data;
    int idx_row_size = src2->nb[1];
    int *gid = (int *)(dst->src[3]->data);
    ...
    for (int col_idx = 0; col_idx < nr1; col_idx++) {
        ...
        int remainder = ne00 % 8;
            // 获取锁
        while (atomic_flag_test_and_set(&g_axpy_q4_lock)) {
            // 如果锁已经被占用，则等待
        }
#if defined(__AVX2__)
        // 使用AVX指令进行向量化计算
...
#endif
        atomic_flag_clear(&g_axpy_q4_lock);
    }
#if defined(_MSC_VER)
    _freea(vec);
#endif
...
//     struct ggml_tensor *src2 = dst->src[2];     // predicator [28672, batch]
//     int chunk = ne00 / 32;                      // src0: [8192, 28*1024], src0分成32份，每份大小
    
//     // parallelize by src0 rows
//     const int64_t dr = (src2->ne[0] + chunk - 1)/chunk;     // 神经元也按每份chunk进行分，分成dr份
//     const int nr = ggml_nrows(src0);
...
//     // src0 ne0[n_embd, n_ff]
//     // src1 ne1[n_ff, n_token]
//     // wdata   [n_ff, n_token]
//     // idx  ne2[n_ff, n_token]
...
static float similarity_cal(const float *new_sparse, const float *old_sparse, int dim) {
    int total = 0;
    int hit = 0;
    for (int i = 0; i < dim; i++) {
        if (new_sparse[i] >= sparse_pred_threshold) {
            total++;
            if (old_sparse[i] >= sparse_pred_threshold) {
                hit++;
            }
        }
    }
...
static void locality_tally(const struct ggml_tensor * src0, const struct ggml_tensor * src1, const struct ggml_tensor * dst) {
    ...
    float *sparse_pred = (float *)(dst->src[2]->data);      // CPU上
    // inter-layer locality
    GGML_ASSERT(g_ffn_neuron_num == dst->src[2]->ne[0] && "error neuron num");
    if (layer != 0) {
        float ratio = similarity_cal(sparse_pred ,g_st_locality.pred_idx[layer], g_ffn_neuron_num);
        g_st_locality.layer_locality.ratio[layer] += ratio;
        g_st_locality.layer_locality.hist[layer][(int)(ratio * 100 / 5)]++;
    } 
    // inter-token locality
    // if (g_token_locality_flag) {
    float ratio = similarity_cal(sparse_pred, g_st_locality.sparse_idx[layer], g_ffn_neuron_num);
    g_st_locality.token_locality.ratio[layer] += ratio;
    g_st_locality.token_locality.hist[layer][(int)(ratio * 100 / 5)]++;
    if (layer == 0) {
        g_st_locality.token_locality.cnt++;
    }
    // }

    memcpy(g_st_locality.sparse_idx[layer], sparse_pred, sizeof(float) * g_ffn_neuron_num);
}
...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    // if(params->ith == 0 && strncmp(ggml_get_name(tensor), "mlp_pre_out", 11) == 0){
    //     // ggml_set_zero(tensor);
    //     char* data = tensor->data;
    //     for (int i = 0; i < tensor->ne[1]; i++) {
    //         float* d = (float *)(data + i * tensor->nb[1]);
    //         for (int j = 0; j < tensor->ne[0]; j++) {
    //             d[j] = 1.0f;
    //         }
    //     }
    //     return;
    //     // printf("compute node %s in gpu, backend: %d, op: %d, extra: %p\n", ggml_get_name(tensor), tensor->backend, tensor->op, tensor->extra);
    // }
    #ifdef GGML_USE_CUBLAS
    bool skip_cpu = ggml_cuda_compute_forward(params, tensor);
    #else
    bool skip_cpu = ggml_cann_compute_forward(params, tensor);
    #endif
    if (skip_cpu) {
        // if(params->ith == 0 && strncmp(ggml_get_name(tensor), "ffn_norm-0", 10) == 0){
        //     printf("compute node %s in gpu, backend: %d, shape: [%d, %d, %d, %d]\n", ggml_get_name(tensor), tensor->backend, tensor->ne[0], tensor->ne[1], tensor->ne[2], tensor->ne[3]);
        // }
        return;
    }

    // if(params->ith == 0 && strncmp(ggml_get_name(tensor), "mlp_pre_out", 11) == 0){
    //     printf("compute node %s in cpu\n", ggml_get_name(tensor));
    //     GGML_ASSERT(false);
    // }
    // if(params->ith == 0 && strncmp(ggml_get_name(tensor), "ffn_norm-0", 10) == 0){
    //     printf("compute node %s in cpu, backend: %d, shape: [%d, %d, %d, %d]\n", ggml_get_name(tensor), tensor->backend, tensor->ne[0], tensor->ne[1], tensor->ne[2], tensor->ne[3]);
    // }
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_AXPY:
            ...
            #ifdef LOCALITY_TEST
                if(params->ith == 0 && params->type == GGML_TASK_COMPUTE) {
                    locality_tally(tensor->src[0], tensor->src[1], tensor);
                }
            #endif
                if (src3 != NULL){
        ...
        case GGML_OP_PRELOAD:
            {
                ggml_compute_forward_preload(params, tensor->src[0], tensor);
            } break;
        case GGML_OP_SCALE:
            {
        ...
        case GGML_OP_CONT:
            {
                #ifdef LOCALITY_TEST
                    if(params->ith == 0 && params->type == GGML_TASK_COMPUTE) {
                        const char *_tmp = "mlp_pre_out_cont";
                        if (strncmp(_tmp, tensor->name, 16) == 0) {
                            memcpy(g_st_locality.pred_idx[tensor->layer+1], tensor->src[0]->data, sizeof(float) * g_ffn_neuron_num);
                        }
                    }
                #endif
        ...
        case GGML_OP_SOFT_MAX_EXP:
            {
                ggml_compute_forward_soft_max_exp(params, tensor->src[0], tensor->src[1], tensor);
            } break;
        case GGML_OP_SOFT_MAX_MAX:
            {
                ggml_compute_forward_soft_max_max(params, tensor->src[0], tensor);
            } break;
        case GGML_OP_SOFT_MAX_SUM:
            {
                ggml_compute_forward_soft_max_sum(params, tensor->src[0], tensor);
            } break;
        case GGML_OP_SOFT_MAX_MERGE:
            {
                ggml_compute_forward_soft_max_merge(params, tensor->src[0], tensor);
            } break;
        case GGML_OP_START:
            {
                ggml_compute_forward_time_start(params, tensor->src[0], tensor);  
            } break;
        case GGML_OP_STOP:
            {
                ggml_compute_forward_time_stop(params, tensor->src[0], tensor);  
            } break;
        case GGML_OP_SYNC_POINT:
            {
                ggml_compute_forward_sync_point(params, tensor->src[0], tensor);
            } break;
    ...
    // if(strncmp(ggml_get_name(tensor), "inp_embd", 8) == 0){
    //     // printf("compute node %s in cpu\n", ggml_get_name(tensor));
    //     GGML_ASSERT(tensor->type == GGML_TYPE_F32);
    //     int64_t n_embd = tensor->ne[0];
    //     int64_t n_token = tensor->ne[1];
    //     printf("inp_embd shape: n_embd %ld, n_token %ld\n", n_embd, n_token);
    //     for(int i=0;i<n_token;i++){
    //         float *data = (float *)tensor->data + i*n_embd;
    //         printf("token %d: ", i);
    //         for(int j=0;j<8;j++){
    //             printf("%f ", data[j]);
    //         }
    //         printf("\n");
    //     }
    //     // printf("compute node %s in cpu, backend: %d, op: %d, extra: %p\n", ggml_get_name(tensor), tensor->backend, tensor->op, tensor->extra);
    //     return;
    // }

    // if(strncmp(tensor->name, "ffn_up_act-0", 12) == 0){
    //     GGML_ASSERT(tensor->backend == GGML_BACKEND_CPU);
    //     int64_t n_token = tensor->ne[1];
    //     int64_t n_ff = tensor->ne[0];
    //     // printf("\ntensor %s, backend: %d, type: %d, shape: [%ld, %ld, %ld, %ld] data: %p, extra: %p\n", tensor->name, tensor->backend, tensor->type, tensor->ne[0], tensor->ne[1], tensor->ne[2], tensor->ne[3], tensor->data, tensor->extra);
    //     for(int i = 0; i < n_token; i++){
    //         for(int j = 0; j < n_ff; j++){
    //             float tmp = ((float *)tensor->data)[i*n_ff + j];
    //             if(tmp>0.0){
    //                 printf("%f(%d) ", tmp, j);
    //             }
    //         }
    ...
    // if((strncmp(tensor->name, "ffn_down_b-0", 12) == 0 ||
    // strncmp(tensor->name, "ffn_down_sparse-0", 17) == 0) && params->type == 1){
    //     debug_cnt++;
    //     if(debug_cnt==params->nth){
    //         struct ggml_tensor* down_result = tensor;
...
#elif defined(GGML_USE_CANN)
                if (ggml_cann_can_mul_mat(node->src[0], node->src[1], node)) {
                    n_tasks = 1; // TODO: this actually is doing nothing
                                 //       the threads are still spinning
                }
#elif defined(GGML_USE_CLBLAST)
...
void active_swap(struct elem *a, struct elem *b) {
    struct elem tmp = *a;
    *a = *b;
    *b = tmp;
}

void heapify(struct elem *arr, int i, int n, bool min_heap) {
    int l = 2 * i + 1;
    int r = 2 * i + 2;
    int min_max_idx = i;
    if (min_heap) {
        if (l < n && arr[l].active_cnt < arr[min_max_idx].active_cnt) {
            min_max_idx = l;
        }
        if (r < n && arr[r].active_cnt < arr[min_max_idx].active_cnt) {
            min_max_idx = r;
        }
    } else {
        if (l < n && arr[l].active_cnt > arr[min_max_idx].active_cnt) {
    ...
    if (min_max_idx != i) {
        active_swap(&arr[i], &arr[min_max_idx]);
        heapify(arr, min_max_idx, n, min_heap);
    }
}

void buildHeap(struct elem *arr, int n, bool min_heap) {
    for (int i = n / 2 - 1; i >= 0; i--) {
        heapify(arr, i, n, min_heap);
    }
}

void topK(struct elem *arr, int n, int k, bool get_max) {
    buildHeap(arr, k, get_max ? true : false);
    for (int i = k; i < n; i++) {
        if (get_max) {
            if (arr[i].active_cnt > arr[0].active_cnt) {
                active_swap(&arr[i], &arr[0]);
...
int active_compare(const void *a, const void *b) {
    return ((struct elem *)b)->active_cnt - ((struct elem *)a)->active_cnt;
}

static int load_neurons(struct ggml_tensor *src, struct ggml_tensor *dst, int *neuron_state, int neuron_num) {
    int replaced_num = 0;
    for (int i = 0; i < neuron_num; i++) {
        // int64_t start_time = ggml_perf_time_us();
        if (neuron_state[i] >= 0) {
            #ifdef GGML_USE_CUBLAS
                ggml_cuda_cpy_ptr_1d(dst, src, i, neuron_state[i]);
            #else
                ggml_cann_cpy_ptr_1d(dst, src, i, neuron_state[i]);
            #endif
            replaced_num++;
        }
        // printf("unit load time: %ld\n", ggml_perf_time_us() - start_time);
    }
    return replaced_num;
}
...
// easy impl only 1 backend thread
extern float computility_up_cpu;
extern float computility_up_gpu;
extern float computility_down_cpu;
extern float computility_down_gpu;

static thread_ret_t ggml_preload_thread(void *data) {
    int test = 0;
    static int g_run_cnt = 0;
#ifdef LOG_PRELOAD
    FILE *analysis_file = fopen("qinfer.log", "a");
    fprintf(analysis_file, "\n\ng_run_cnt: %d\n", g_run_cnt++);
#endif

    while (true) {
        if (atomic_load(&has_new_preload_task)) {
            // process
            struct ggml_tensor *dst = preload_src_tensor;
            int64_t start_time = ggml_perf_time_us();

            const struct ggml_tensor *sparse_tensor = dst->src[0];
            if (sparse_tensor == NULL) {
                atomic_store(&has_new_preload_task, false);
                continue;
            }
            struct ggml_tensor *gpu_idx = dst->src[1];
            struct ggml_tensor *gpu_bucket = dst->src[2];
            struct ggml_tensor *ffn_gate_gpu = dst->src[3];
            struct ggml_tensor *ffn_up_gpu = dst->src[4];
            struct ggml_tensor *ffn_down_gpu = dst->src[5];
            struct ggml_tensor *gpu_idx_down = ffn_down_gpu->src[1];
            struct ggml_tensor *gpu_bucket_down = ffn_down_gpu->src[2];

            int64_t neuron_num = sparse_tensor->ne[0];
            int64_t batch_num = sparse_tensor->ne[1];

            int64_t cached_size = gpu_bucket->ne[0];
            int64_t cached_size_down = gpu_bucket_down->ne[0];
            int64_t offload_num = cached_size;
            int64_t offload_num_down = cached_size_down;

            int32_t *idx_data = ggml_get_data_i32(gpu_idx);
            int32_t *bucket_data = (int32_t *)gpu_bucket->data;
            int32_t *idx_down_data = ggml_get_data_i32(gpu_idx_down);
            int32_t *bucket_down_data = (int32_t *)gpu_bucket_down->data;
            if (batch_num <= 512 && offload_num > 0) {
                // goto process_finish;
                if (sparse_tensor->backend == GGML_BACKEND_GPU) {
                    #ifdef GGML_USE_CUBLAS
                        ggml_cuda_copy_to_host(dst);
                    #else
                        ggml_cann_copy_to_host(dst);
                    #endif
                }

                float *sparse_idx = (float *)sparse_tensor->data;
                int pred_active_cnt[neuron_num];
                struct elem cpu_array[neuron_num];
                struct elem gpu_array[cached_size];
                struct elem gpu_array_down[cached_size_down];

                // init
                for (int j = 0; j < neuron_num; j++) {
                    pred_active_cnt[j] = 0;
                    cpu_array[j].idx = j; // row idx of weight matrix
                    cpu_array[j].active_cnt = 0;
                    if (j < cached_size) {
                        gpu_array[j].idx = j; // row idx of gpu_bucket
                        gpu_array[j].active_cnt = 0;
                    }
                    if (j < cached_size_down) {
                        gpu_array_down[j].idx = j; // row idx of gpu_bucket_down
                        gpu_array_down[j].active_cnt = 0;
                    }
                }
                // count total active cnt
                int totalActiveCnt = 0;
                for (int i = 0; i < batch_num; i++) {
                    for (int j = 0; j < neuron_num; j++) {
                        if (sparse_idx[i * neuron_num + j] >= sparse_pred_threshold) {
                            cpu_array[j].active_cnt++;
                            pred_active_cnt[j]++;
                            totalActiveCnt++;
                        }
                    }
                }
                // count gpu active cnt
                int gpuActiveCnt = 0;
                int gpuActiveCnt_down = 0;
                for (int i = 0; i < cached_size; i++) {
                    int row_i = bucket_data[i];
                    gpu_array[i].active_cnt = cpu_array[row_i].active_cnt;
                    gpuActiveCnt += cpu_array[row_i].active_cnt;
                }
                for (int i = 0; i < cached_size_down; i++) {
                    int row_i = bucket_down_data[i];
                    gpu_array_down[i].active_cnt = cpu_array[row_i].active_cnt;
                    gpuActiveCnt_down += cpu_array[row_i].active_cnt;
                }
                int cpuActiveCnt = totalActiveCnt - gpuActiveCnt;
                int cpuActiveCnt_down = totalActiveCnt - gpuActiveCnt_down;

                bool offload_more_to_gpu = cpuActiveCnt / computility_up_cpu > gpuActiveCnt / computility_up_gpu;
                // start_time = ggml_perf_time_us();
                topK(cpu_array, neuron_num, offload_num, offload_more_to_gpu);
                topK(gpu_array, cached_size, offload_num, !offload_more_to_gpu);
                // printf("sort time: %ld\n", ggml_perf_time_us() - start_time);

                // start_time = ggml_perf_time_us();
                enum STATE_OF_NEURON { DEFAULT = -3, NEED_TO_OFFLOAD = -2, ALREADY_OFFLOAD = -1 };
                int neuron_state[neuron_num];
                for (int i = 0; i < neuron_num; i++) {
                    neuron_state[i] = DEFAULT;
                }
                for (int i = 0; i < offload_num; i++) {
                    neuron_state[cpu_array[i].idx] = NEED_TO_OFFLOAD;
                }
                for (int i = 0; i < cached_size; i++) {
                    if (neuron_state[bucket_data[i]] == NEED_TO_OFFLOAD) {
                        neuron_state[bucket_data[i]] = ALREADY_OFFLOAD;
                    }
                }
                ...
                int cnt_already = 0;
                for (int i = 0; i < neuron_num; i++) {
                    if (neuron_state[i] == NEED_TO_OFFLOAD) {
                        cnt_need++;
                    } else if (neuron_state[i] == ALREADY_OFFLOAD) {
                        cnt_already++;
                    }
                }
                GGML_ASSERT(cnt_need + cnt_already == offload_num);

                // do move
                int cur = 0;
                int has_offloaded = 0;
                int new_cpu_active_cnt = cpuActiveCnt;
                int new_gpu_active_cnt = gpuActiveCnt;
                for (int i = 0; i < offload_num && has_offloaded < cnt_need; i++) {
                    int bucket_idx = gpu_array[i].idx;
                    int neuron_idx = bucket_data[bucket_idx];
                    if (neuron_state[neuron_idx] == DEFAULT) {
                        // need to be replaced
                        while (cur < neuron_num && neuron_state[cur] != NEED_TO_OFFLOAD) {
                            cur++;
                        }
                        GGML_ASSERT(cur < neuron_num && "error: cur out of range");
                        idx_data[neuron_idx] = 0;
                        bucket_data[bucket_idx] = cur;
                        idx_data[cur] = 1;
                        neuron_state[cur] = bucket_idx;
                        if (ffn_gate_gpu != NULL) {
                            #ifdef GGML_USE_CUBLAS
                                ggml_cuda_cpy_ptr_1d(ffn_gate_gpu, ffn_gate_gpu->src[0], cur, bucket_idx);
                            #else
                                ggml_cann_cpy_ptr_1d(ffn_gate_gpu, ffn_gate_gpu->src[0], cur, bucket_idx);
                            #endif
                        }
                        #ifdef GGML_USE_CUBLAS
                            ggml_cuda_cpy_ptr_1d(ffn_up_gpu, ffn_up_gpu->src[0], cur, bucket_idx);
                        #else
                            ggml_cann_cpy_ptr_1d(ffn_up_gpu, ffn_up_gpu->src[0], cur, bucket_idx);
                        #endif
                        new_gpu_active_cnt = new_gpu_active_cnt - gpu_array[i].active_cnt + pred_active_cnt[cur];
                        new_cpu_active_cnt = new_cpu_active_cnt + gpu_array[i].active_cnt - pred_active_cnt[cur];
                        cur++;
                        has_offloaded++;
                    }
                    if (atomic_load(&is_waiting)) {
                        break;
                    }
                    if ((offload_more_to_gpu &&
                         (new_cpu_active_cnt / computility_up_cpu < new_gpu_active_cnt / computility_up_gpu)) ||
                        (!offload_more_to_gpu &&
                         (new_cpu_active_cnt / computility_up_cpu > new_gpu_active_cnt / computility_up_gpu))) {
                        break;
                    }
                ...
                bool offload_more_to_gpu_down = cpuActiveCnt_down / computility_down_cpu > gpuActiveCnt_down / computility_down_gpu;
                topK(cpu_array, neuron_num, offload_num_down, offload_more_to_gpu_down);
                topK(gpu_array_down, cached_size_down, offload_num_down, !offload_more_to_gpu_down);
                for (int i = 0; i < neuron_num; i++) {
                    neuron_state[i] = DEFAULT;
                }
                for (int i = 0; i < offload_num_down; i++) {
                    neuron_state[cpu_array[i].idx] = NEED_TO_OFFLOAD;
                }
                for (int i = 0; i < cached_size_down; i++) {
                    if (neuron_state[bucket_down_data[i]] == NEED_TO_OFFLOAD) {
                        neuron_state[bucket_down_data[i]] = ALREADY_OFFLOAD;
                    }
                }
                cnt_need = 0;
                cnt_already = 0;
                int new_cpu_active_cnt_down = cpuActiveCnt_down;
                int new_gpu_active_cnt_down = gpuActiveCnt_down;
                for (int i = 0; i < neuron_num; i++) {
                    if (neuron_state[i] == NEED_TO_OFFLOAD) {
                        cnt_need++;
                    } else if (neuron_state[i] == ALREADY_OFFLOAD) {
                ...
                GGML_ASSERT(cnt_need + cnt_already == offload_num_down);

                cur = 0;
                has_offloaded = 0;
                for (int i = 0; i < offload_num_down && has_offloaded < cnt_need; i++) {
                    int bucket_idx = gpu_array_down[i].idx;
                    int neuron_idx = bucket_down_data[bucket_idx];
                    if (neuron_state[neuron_idx] == DEFAULT) {
                        // need to be replaced
                        while (cur < neuron_num && neuron_state[cur] != NEED_TO_OFFLOAD) {
                            cur++;
                        ...
                        GGML_ASSERT(cur < neuron_num && "error: cur out of range");
                        idx_down_data[neuron_idx] = 0;
                        bucket_down_data[bucket_idx] = cur;
                        idx_down_data[cur] = 1;
                        neuron_state[cur] = bucket_idx;
                        #ifdef GGML_USE_CUBLAS
                            ggml_cuda_cpy_ptr_1d(ffn_down_gpu, ffn_down_gpu->src[0], cur, bucket_idx);
                        #else
                            ggml_cann_cpy_ptr_1d(ffn_down_gpu, ffn_down_gpu->src[0], cur, bucket_idx);
                        #endif
                        new_gpu_active_cnt_down =
                            new_gpu_active_cnt_down - gpu_array_down[i].active_cnt + pred_active_cnt[cur];
                        new_cpu_active_cnt_down =
                            new_cpu_active_cnt_down + gpu_array_down[i].active_cnt - pred_active_cnt[cur];
                        cur++;
                        has_offloaded++;
                    }
                    if (atomic_load(&is_waiting)) {
                    ...
                    if ((offload_more_to_gpu_down &&
                         (new_cpu_active_cnt_down / computility_down_cpu < new_gpu_active_cnt_down / computility_down_gpu)) ||
                        (!offload_more_to_gpu_down &&
                         (new_cpu_active_cnt_down / computility_down_cpu > new_gpu_active_cnt_down / computility_down_gpu))) {
                        break;
                    }
                }

                // synchronous copy, async copy by stream 0 will faster?
                #ifdef GGML_USE_CUBLAS
                    ggml_cuda_copy_to_device(gpu_bucket);
                    ggml_cuda_copy_to_device(gpu_bucket_down);
                    // ggml_stream_synchronize(0); // need to synchronize stream 0?
                    ggml_stream_synchronize(1);
                #else
                    ggml_cann_copy_to_device(gpu_bucket);
                    ggml_cann_copy_to_device(gpu_bucket_down);
                    ggml_cann_stream_synchronize(1);
                #endif
                ...
                // log replace info
                size_t idx = 0;
                while (idx < strlen(dst->name)) {
                    if (dst->name[idx] == '-') {
                        break;
                    }
                    idx++;
                }
                int layer_idx = atoi(&dst->name[idx + 1]) + 1;
                fprintf(analysis_file, "%-17s do preload for layer %-2d, batch_size: %ld, active_cnt(cpu:gpu): up(%d:%d)->(%d:%d) "
                       "down(%d:%d)->(%d:%d)\n",
                       dst->name, layer_idx, batch_num, cpuActiveCnt, gpuActiveCnt, new_cpu_active_cnt,
                       new_gpu_active_cnt, cpuActiveCnt_down, gpuActiveCnt_down, new_cpu_active_cnt_down,
                       new_gpu_active_cnt_down);
#endif
            }
        ...
        if (atomic_load(&is_over_preload) == true) {
            // printf("preload thread exit\n");
            break;
        }
    }
#ifdef LOG_PRELOAD
        ...
        if (atomic_fetch_sub(&state->shared->n_active, 1) == 1) {
            ...
            while (++node_n < cgraph->n_nodes) {
                ...
                if (GGML_OP_HAS_INIT[node->op]) {
                    ...
                    size_t start_us = ggml_time_us();
                    ggml_compute_forward(&params, node);
                    size_t end_us = ggml_time_us();
                    if (node->op == GGML_OP_MUL_MAT) {
                        stat_record(global_stat.layer_stat[node->layer], "quant", end_us - start_us, GGML_BACKEND_CPU, GGML_BACKEND_CPU);
                    }
                }
...
int ggml_graph_compute(struct ggml_cgraph * cgraph, struct ggml_cplan * cplan) {
    ...
    atomic_store(&has_new_preload_task, false);
    atomic_store(&is_over_preload, false);
    atomic_store(&is_waiting, false);
    ggml_thread_t backend_preload_thread;
    const int brc = ggml_thread_create(&backend_preload_thread, NULL, ggml_preload_thread, NULL);
    GGML_ASSERT(brc == 0);
    UNUSED(brc);
    ...
    // join or kill preload thread
    {
        atomic_store(&is_over_preload, true);
        const int rc = ggml_thread_join(backend_preload_thread, NULL);
        GGML_ASSERT(rc == 0);
    }
