...
extern int64_t benchmark_n_batch;
extern int store_in_gpu_step;
extern int store_in_cpu_step;
bool gpt_params_parse(int argc, char ** argv, gpt_params & params) {
    ...
    benchmark_n_batch = params.n_parallel;
    store_in_gpu_step = params.store_in_gpu_step;
    store_in_cpu_step = params.store_in_cpu_step;
    return result;
}
...
bool gpt_params_parse_ex(int argc, char ** argv, gpt_params & params) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        } else if (arg == "--kvcache-mode") {
            if (++i >= argc) {
                invalid_param = true;
                break;
            }
            params.kvcache_mode = std::stoi(argv[i]);
        } else if (arg == "--kvcache-ratio") {
            if (++i >= argc) {
                invalid_param = true;
                break;
            }
            params.kvcache_ratio = std::stof(argv[i]);
        } else if (arg == "--kvcache-step") {
            if (++i >= argc) {
                invalid_param = true;
                break;
            }
            params.kvcache_step = std::stof(argv[i]);
        } else if (arg == "--disable-kvc-dynamic") {
            params.kvcache_dynamic = false;
        } else if (arg == "--keep") {
            if (++i >= argc) {
                invalid_param = true;
                break;
...
#endif
        } else if (arg == "--store-in-gpu-step") {
            if (++i >= argc) {
                invalid_param = true;
                break;
            }
            params.store_in_gpu_step = std::stoi(argv[i]);
        } else if (arg == "--store-in-cpu-step") {
            if (++i >= argc) {
                invalid_param = true;
                break;
            }
            params.store_in_cpu_step = std::stoi(argv[i]);
...
struct llama_model_params llama_model_params_from_gpt_params(const gpt_params & params) {
    ...
    mparams.kvcache_mode     = params.kvcache_mode;
    mparams.kvcache_ratio     = params.kvcache_ratio;
    mparams.kvcache_step     = params.kvcache_step;
    g_kvc_split.flag        = params.kvcache_dynamic;
    return mparams;
}
...
struct llama_context_params llama_context_params_from_gpt_params(const gpt_params & params) {
    ...
    cparams.batch_size        = params.n_parallel;
